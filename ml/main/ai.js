...
function generalAI(i) {
  ...
  if ((player[i].currentSubaction == "LEFT" || player[i].currentSubaction == "RIGHT") && player[i].currentAction == "NONE") {
	  player[i].currentSubaction = "NONE";
  }
  ...
  if (player[i].difficulty >= 2) {
	  ...
	  //if (a % 20 == 0) {console.log(isAboveGround(player[i].phys.pos.x,player[i].phys.pos.y + 1.0));}
	  if (player[i].currentAction == "NONE" && ["OTTOTTOWAIT","WAIT","SMASHTURN","WALKF","WALK","SQUAT"].indexOf(player[i].actionState) != -1 && isAboveGround(player[i].phys.pos.x,player[i].phys.pos.y + 1.0)[1] == "platform" && player[i].phys.grounded && player[i].phys.pos.y - player[nearest].phys.pos.y > 0 && Math.abs(player[nearest].phys.pos.x - player[i].phys.pos.x) <= 40) {
		  //is above platform
		  //console.log("H");
  ...
  if (player[i].currentAction == "LEDGESTALL") {
	  ...
	  if (["CLIFFWAIT","JUMPF","FALL","FALLAERIAL","JUMPAERIAL","JUMPAERIALF","JUMPAERIAL1","JUMPAERIALB"].indexOf(player[i].actionState) == -1) {
		  player[i].currentAction = "NONE";
		  player[i].currentSubaction = "NONE";
	  }
  ...
  if (player[i].currentAction == "LEDGEDASH") {
	  if(["CLIFFWAIT","JUMPAERIALF","JUMPAERIALB","FALLAERIAL","ESCAPEAIR","FALL","JUMPAERIAL1","JUMPAERIAL"].indexOf(player[i].actionState) == -1) {
		  player[i].currentAction = "NONE";
	  } else {
  ...
  if (player[i].actionState != "DOWNWAIT") {
    ...
    if ((player[i].currentAction == "NONE" && (player[i].actionState == "CLIFFWAIT")) || (player[i].currentAction == "LEDGEDASH"|| player[i].currentAction == "LEDGEAIRATTACK2" || player[i].currentAction == "LEDGEAIRATTACK" || player[i].currentAction == "LEDGEGETUP" || player[i].currentAction == "LEDGEATTACK" || player[i].currentAction == "LEDGEJUMP" || player[i].currentAction == "LEDGEROLL" || player[i].currentAction == "LEDGEJUMP" || player[i].currentAction == "TOURNAMENTWINNER")) {
	  var inputs = CPULedge(player[i],i);
      //do inputs
      player[i].inputs.lStickAxis[0].x = inputs.lstickX;
      player[i].inputs.lStickAxis[0].y = inputs.lstickY;
  ...
  if (player[i].currentAction == "NONE" && player[i].currentSubaction == "NONE" && (player[i].actionState == "WAIT" || player[i].actionState == "OTTOTTOWAIT" || player[i].actionState == "WALK")) {  //walk towards enemy
...
function foxAI(i) {
  if (player[i].currentAction == "LEDGESTALL") {
	...
	if (player[i].currentAction == "RESPAWNMULTISHINE") {
	  ...
	  if ((["DOWNSPECIALGROUND","DOWNSPECIALAIR","KNEEBEND","JUMPF","JUMPB","WAIT","WALK","WALKF","OTTOTTOWAIT"].indexOf(player[i].actionState) == -1.0)) {
		  player[i].currentAction = "NONE";
		  player[i].currentSubaction = "NONE";
	  }
	  if (player[i].currentSubaction == "SHINE") {
		  ...
		  player[i].inputs.b[0] = true;
		  if ((player[i].timer == 3 && player[i].actionState == "DOWNSPECIALGROUND") || (player[i].timer == 6 && player[i].actionState == "DOWNSPECIALGROUND")) {
			  player[i].inputs.x[0] = true;
...
//ledgepos is where a character can grab the ledge
  ...
  if (cpu.actionState.substr(0,4) == "JUMP" || cpu.actionState == "FALLAERIAL" || cpu.actionState == "DAMAGEFALL" || cpu.actionState == "FALL" || cpu.actionState == "FALLSPECIAL") {
    ...
    if (cpu.phys.cVel.y <= 0 && ((closest.y - cpu.phys.pos.y > 10 && (Math.abs(closest.x - cpu.phys.pos.x) > 25)) || (closest.y - cpu.phys.pos.y > 25 && (Math.abs(closest.x - cpu.phys.pos.x) <= 25)))) { //is falling
