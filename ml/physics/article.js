...
"ILLUSION" : {
  ...
  main : function(i){
    ...
    aArticles[i][2].ecb = [new Vec2D(player[p].phys.posPrev.x,player[p].phys.posPrev.y-10),new Vec2D(player[p].phys.posPrev.x+10,player[p].phys.posPrev.y),new Vec2D(player[p].phys.posPrev.x,player[p].phys.posPrev.y+10),new Vec2D(player[p].phys.posPrev.x-10,player[p].phys.posPrev.y)];
...
function articleHurtCollision(a,v,previous){
  ...
  if (distance.x > (hurtWidth/2 + aArticles[a][2].hb.size)) { return false; }
  if (distance.y > (hurtHeight/2 + aArticles[a][2].hb.size)) { return false; }
