...
function interpolatedHitCircleCollision(circlePos,r,p,j){
  ...
  var segment4 = new Segment2D(circlePos.x,circlePos.y,(segment3.x+segment3.vecx)-circlePos.x,(segment3.y+segment3.vecy)-circlePos.y);
...
function interpolatedHitHurtCollision(i,p,j,phantom){
  ...
  else if (segmentSegmentCollision(new Vec2D(hurt.min.x,hurt.min.y),new Vec2D(hurt.min.x,hurt.max.y),hb[0],hb[1]) || segmentSegmentCollision(new Vec2D(hurt.min.x,hurt.min.y),new Vec2D(hurt.min.x,hurt.max.y),hb[2],hb[3])){
    return true;
  }
  else if (segmentSegmentCollision(new Vec2D(hurt.min.x,hurt.max.y),new Vec2D(hurt.max.x,hurt.max.y),hb[0],hb[1]) || segmentSegmentCollision(new Vec2D(hurt.min.x,hurt.max.y),new Vec2D(hurt.max.x,hurt.max.y),hb[2],hb[3])){
    return true;
  }
  else if (segmentSegmentCollision(new Vec2D(hurt.max.x,hurt.max.y),new Vec2D(hurt.max.x,hurt.min.y),hb[0],hb[1]) || segmentSegmentCollision(new Vec2D(hurt.max.x,hurt.max.y),new Vec2D(hurt.max.x,hurt.min.y),hb[2],hb[3])){
    return true;
  }
  else {
    if (phantom){
...
function hitHurtCollision(i,p,j,previous,phantom){
  ...
  var hurtCenter = new Vec2D((player[i].phys.hurtbox.min.x+player[i].phys.hurtbox.max.x)/2,(player[i].phys.hurtbox.min.y+player[i].phys.hurtbox.max.y)/2);
...
function executeHits(){
  ...
  for (var i=0;i<hitQueue.length;i++){
    ...
    if (gameMode == 2){
      if (shieldHit){
        ...
        player[a].rpsPoints++;
        player[v].hit.hitlag = Math.floor(damage * (1/3) + 3);
        player[v].hit.knockback = getKnockback(player[a].hitboxes.id[h],damage,damage,0,player[v].charAttributes.weight,false,false);
  ...
  for (var j=0;j<grabQueue.length;j++){
    if (!ignoreGrabs[grabQueue[j][0]]){
      if (!grabQueue[j][2]){
        if (player[grabQueue[j][1]].actionState == "GRAB" && player[grabQueue[j][1]].timer > 0 && player[grabQueue[j][1]].timer < 14 && player[grabQueue[j][1]].phys.face != player[grabQueue[j][0]].phys.face){
      ...
      if (!grabQueue[j][2]){
        var a = grabQueue[j][0];
        var v = grabQueue[j][1];
        if (player[v].phys.grabbedBy == -1 && player[a].phys.grabbing == -1 && player[v].phys.hurtBoxState == 0 && !player[v].phys.grabTech){
...
function executeGrabTech(a,v){
  ...
  sounds.parry.play();
  drawVfx("shieldup",new Vec2D((player[a].phys.pos.x+player[v].phys.pos.x)/2,player[a].phys.pos.y+12),player[v].phys.face,3);
}
