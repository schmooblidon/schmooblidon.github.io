...
furacry:new Howl({src:["sfx/furacry.wav"],volume:.4}),furaloop:new Howl({src:["sfx/furaloop.wav"],loop:!0}),parry:new Howl({src:["sfx/parry.wav"]})};function playSfx(a){sounds[a].play()}
...
function executeHits(){for(var a=0;a<hitQueue.length;a++){var b=hitQueue[a][0],d=hitQueue[a][1],e=hitQueue[a][2],f=hitQueue[a][3],n=hitQueue[a][4],q=hitQueue[a][5];72==player[b].actionState&&sounds.furaloop.stop(player[b].furaLoopID);if(2==player[d].hitboxes["id"+e].type)console.log(player[b].actionState),73==player[b].actionState&&3<player[b].timer&&9>player[b].timer?(player[d].phys.pos.x<player[b].phys.pos.x?(player[d].phys.face=1,player[b].phys.face=-1):(player[d].phys.face=-1,player[b].phys.face=
1),player[d].phys.grabTech=!0,player[b].phys.grabTech=!0,aS[77].init(d),aS[77].init(b),playSfx("parry"),drawVfx("powerShield",new Vec2D((player[d].phys.pos.x+player[b].phys.pos.x)/2,player[d].phys.pos.y+12),player[b].phys.face,3)):-1!=player[b].phys.grabbedBy||-1!=player[d].phys.grabbing||0!=player[b].phys.hurtBoxState||player[b].phys.grabTech||(player[b].phys.cVel=new Vec2D(0,0),player[b].phys.kVel=new Vec2D(0,0),player[d].phys.cVel=new Vec2D(0,0),player[d].phys.kVel=new Vec2D(0,0),player[b].phys.grabbedBy=
d,player[b].phys.shielding=!1,player[d].phys.grabbing=b,aS[74].init(b));else{var h=player[d].hitboxes["id"+e].dmg;player[b].phys.grabTech=!1;player[d].phys.charging&&(h*=1+.3671/60*player[d].phys.chargeFrames);player[d].hit.hitlag=Math.floor(1/3*h+3);if(f){if(!player[b].phys.powerShieldActive&&(player[b].phys.shieldHP-=h,0>player[b].phys.shieldHP)){player[b].phys.shielding=!1;player[b].phys.cVel.y=2.5;player[b].phys.grounded=!1;player[b].phys.shieldHP=0;drawVfx("breakShield",player[b].phys.pos,player[b].phys.face);
aS[69].init(b);break}player[b].hit.hitlag=Math.floor(1/3*h+3);e=.6;player[b].phys.powerShieldActive&&(e=1,player[b].phys.powerShielded=!0,player[b].hit.powershield=!0,drawVfx("powerShield",player[b].phys.shieldPositionReal,player[b].phys.face,player[b].phys.shieldSize));player[b].hit.shieldstun=Math.floor(h)*(.65*(1-(player[b].phys.shieldAnalog-.3)/.7)+.3)*1.5+2;e*=Math.floor(h)*(.195*(1-(player[b].phys.shieldAnalog-.3)/.7)+.09)+.4;2<e&&(e=2);h=.1*Math.floor(h)*(player[b].phys.shieldAnalog-.3)+.02;
player[d].phys.pos.x<player[b].phys.pos.x?(player[b].phys.cVel.x+=e,player[d].phys.cVel.x-=h):(player[b].phys.cVel.x-=e,player[d].phys.cVel.x+=h)}else 0==player[b].phys.hurtBoxState&&(player[b].hit.knockback=getKnockback(player[d].hitboxes["id"+e],h,h,player[b].percent,player[b].charAttributes.weight),player[b].hit.angle=player[d].hitboxes["id"+e].angle,361==player[b].hit.angle&&(32.1>player[b].hit.knockback?player[b].hit.angle=0:32.1<=player[b].hit.knockback&&(player[b].hit.angle=44)),player[b].hit.hitlag=
Math.floor(1/3*h+3),n?(player[b].phys.thrownHitbox=!0,player[b].phys.thrownHitboxOwner=d,player[b].phys.pos=new Vec2D(player[d].phys.pos.x+player[d].hitboxes["id"+e].offset.x*player[d].phys.face,player[d].phys.pos.y+player[d].hitboxes["id"+e].offset.y),player[b].phys.grabbedBy=-1,player[b].hit.hitlag=1,player[d].hit.hitlag=1,player[b].hit.reverse=1==player[d].phys.face?!1:!0,q&&(playSfx("bounce"),drawVfx("groundBounce",player[b].phys.pos,player[b].phys.face))):(player[b].hit.hitPoint=new Vec2D(player[d].phys.pos.x+
player[d].hitboxes["id"+e].offset[player[d].hitboxes.frame].x*player[d].phys.face,player[d].phys.pos.y+player[d].hitboxes["id"+e].offset[player[d].hitboxes.frame].y),player[d].phys.pos.x<player[b].phys.pos.x?(player[b].hit.reverse=!1,player[b].phys.face=-1):(player[b].hit.reverse=!0,player[b].phys.face=1)),player[b].percent+=h,-1==player[b].phys.grabbedBy||44<player[b].hit.knockback?(player[b].hit.hitstun=getHitstun(player[b].hit.knockback),80<=player[b].hit.knockback||n?aS[39].init(b,!n):aS[41].init(b)):
aS[80].init(b),player[b].phys.grounded&&180<player[b].hit.angle&&80<=player[b].hit.knockback&&(playSfx("bounce"),drawVfx("groundBounce",player[b].phys.pos,player[b].phys.face),player[b].hit.angle=360-player[b].hit.angle,player[b].hit.knockback*=.8),screenShake(player[b].hit.knockback),percentShake(player[b].hit.knockback,b),n?playSfx("stronghit"):0==player[d].hitboxes["id"+e].type?playSfx("normalweakhit"):50>player[b].hit.knockback?playSfx("swordweakhit"):100>player[b].hit.knockback?playSfx("swordmediumhit"):
140>player[b].hit.knockback?playSfx("swordstronghit"):(playSfx("swordreallystronghit"),playSfx("cheer"),280>player[b].hit.knockback?(playSfx("stronghit"),playSfx("weakhurt")):(playSfx("strongerhit"),playSfx("stronghurt"))))}}}function getKnockback(a,b,d,e,f){a=0==a.sk?.01*a.kg*(1.4*(.05*d*(b+Math.floor(e))+.1*(b+Math.floor(e)))*(2-.02*f/(1+.01*f))+18)+a.bk:a.kg/100*(200/(f+100)*(10*a.sk/20+1)*1.4+18)+a.bk;2500<a&&(a=2500);return a}
...
Math.sin(b)}else player[a].phys.cVel.x=0,player[a].phys.cVel.y=0;player[a].phys.fastfalled=!1;player[a].phys.landingMultiplier=3;aS[11].main(a)},main:function(a){player[a].timer++;aS[11].interrupt(a)||(30>player[a].timer?(player[a].phys.cVel.x*=.9,player[a].phys.cVel.y*=.9):(airDrift(a),fastfall(a)),4==player[a].timer&&(player[a].phys.intangibleTimer=25,playSfx("airdodge")))},interrupt:function(a){return 49<player[a].timer?(aS[14].init(a),!0):!1}},{name:"LANDINGFALLSPECIAL",canEdgeCancel:!0,init:function(a){player[a].actionState=
12;player[a].timer=0;drawVfx("circleDust",player[a].phys.pos,player[a].phys.face);playSfx("land");aS[12].main(a)},main:function(a){player[a].timer+=player[a].phys.landingMultiplier;aS[12].interrupt(a)||reduceByTraction(a,!0)},interrupt:function(a){return 30<player[a].timer?(aS[0].init(a),!0):!1}},{name:"FALL",canPassThrough:!0,canGrabLedge:[!0,!1],wallJumpAble:!0,init:function(a){player[a].actionState=13;player[a].timer=0;aS[13].main(a)},main:function(a){player[a].timer++;aS[13].interrupt(a)||(fastfall(a),
airDrift(a))},interrupt:function(a){var b=checkForAerials(a),d=checkForSpecials(a);return b[0]?(aS[b[1]].init(a),!0):player[a].inputs.l[0]&&!player[a].inputs.l[1]||player[a].inputs.r[0]&&!player[a].inputs.r[1]?(aS[11].init(a),!0):(player[a].inputs.x[0]&&!player[a].inputs.x[1]||player[a].inputs.y[0]&&!player[a].inputs.y[1]||.7<player[a].inputs.lStickAxis[0].y&&.7>=player[a].inputs.lStickAxis[1].y)&&!player[a].phys.doubleJumped?(aS[19].init(a),!0):d[0]?(aS[d[1]].init(a),!0):10<player[a].timer?(aS[13].init(a),
!0):!1}},{name:"FALLSPECIAL",canPassThrough:!0,canGrabLedge:[!0,!1],wallJumpAble:!1,init:function(a){player[a].actionState=14;player[a].timer=0;aS[14].main(a)},main:function(a){player[a].timer++;aS[14].interrupt(a)||(fastfall(a),airDrift(a))},interrupt:function(a){return 10<player[a].timer?(aS[14].init(a),!0):!1}},{name:"JUMPB",canPassThrough:!0,canGrabLedge:[!0,!1],wallJumpAble:!0,init:function(a,b){player[a].actionState=15;player[a].timer=0;player[a].phys.cVel.y=b?player[a].phys.cVel.y+player[a].charAttributes.fHopInitV:
player[a].phys.cVel.y+player[a].charAttributes.sHopInitV;player[a].phys.cVel.x=player[a].phys.cVel.x*player[a].charAttributes.groundToAir+player[a].inputs.lStickAxis[0].x*player[a].charAttributes.jumpHmaxV;Math.abs(player[a].phys.cVel.x)>player[a].charAttributes.jumpHmaxV&&(player[a].phys.cVel.x=player[a].charAttributes.jumpHmaxV*Math.sign(player[a].phys.cVel.x));player[a].phys.grounded=!1;playSfx("jump");playSfx("jump2");aS[15].main(a)},main:function(a){player[a].timer++;!aS[15].interrupt(a)&&1<
player[a].timer&&(fastfall(a),airDrift(a))},interrupt:function(a){var b=checkForAerials(a),d=checkForSpecials(a);return b[0]?(aS[b[1]].init(a),!0):player[a].inputs.l[0]&&!player[a].inputs.l[1]||player[a].inputs.r[0]&&!player[a].inputs.r[1]?(aS[11].init(a),!0):(player[a].inputs.x[0]&&!player[a].inputs.x[1]||player[a].inputs.y[0]&&!player[a].inputs.y[1]||.7<player[a].inputs.lStickAxis[0].y&&.7>=player[a].inputs.lStickAxis[1].y)&&!player[a].phys.doubleJumped?(aS[19].init(a),!0):d[0]?(aS[d[1]].init(a),
!0):55<player[a].timer?(aS[13].init(a),!0):!1}},{name:"SQUAT",canEdgeCancel:!0,init:function(a){player[a].actionState=16;player[a].timer=0;aS[16].main(a)},main:function(a){player[a].timer++;aS[16].interrupt(a)||reduceByTraction(a,!0)},interrupt:function(a){var b=checkForSpecials(a),d=checkForTilts(a),e=checkForSmashes(a);return 4==player[a].timer&&(-.65>player[a].inputs.lStickAxis[0].y||-.65>player[a].inputs.lStickAxis[1].y||-.65>player[a].inputs.lStickAxis[2].y)&&-.3<player[a].inputs.lStickAxis[6].y&&
-1<player[a].phys.onSurface?(aS[20].init(a),!0):player[a].inputs.l[0]||player[a].inputs.r[0]||0<player[a].inputs.lAnalog[0]||0<player[a].inputs.rAnalog[0]?(aS[21].init(a),!0):b[0]?(aS[b[1]].init(a),!0):e[0]?(aS[e[1]].init(a),!0):d[0]?(aS[d[1]].init(a),!0):7<player[a].timer?(aS[17].init(a),!0):checkForJump(a)?(aS[8].init(a),!0):!1}},{name:"SQUATWAIT",canEdgeCancel:!0,init:function(a){player[a].actionState=17;player[a].timer=0;aS[17].main(a)},main:function(a){player[a].timer++;aS[17].interrupt(a)||
reduceByTraction(a,!0)},interrupt:function(a){var b=checkForSpecials(a),d=checkForTilts(a),e=checkForSmashes(a);return-.61<player[a].inputs.lStickAxis[0].y?(aS[18].init(a),!0):checkForJump(a)?(aS[8].init(a),!0):player[a].inputs.l[0]||player[a].inputs.r[0]||0<player[a].inputs.lAnalog[0]||0<player[a].inputs.rAnalog[0]?(aS[21].init(a),!0):b[0]?(aS[b[1]].init(a),!0):e[0]?(aS[e[1]].init(a),!0):d[0]?(aS[d[1]].init(a),!0):checkForDash(a)?(aS[1].init(a),!0):checkForSmashTurn(a)?(aS[3].init(a),!0):!1}},{name:"SQUATRV",
canEdgeCancel:!0,init:function(a){player[a].actionState=18;player[a].timer=0;aS[18].main(a)},main:function(a){player[a].timer++;aS[18].interrupt(a)||reduceByTraction(a,!0)},interrupt:function(a){var b=checkForSpecials(a),d=checkForTilts(a),e=checkForSmashes(a);return 8<player[a].timer?(aS[0].init(a),!0):checkForJump(a)?(aS[8].init(a),!0):player[a].inputs.l[0]||player[a].inputs.r[0]||0<player[a].inputs.lAnalog[0]||0<player[a].inputs.rAnalog[0]?(aS[21].init(a),!0):b[0]?(aS[b[1]].init(a),!0):e[0]?(aS[e[1]].init(a),
!0):d[0]?(aS[d[1]].init(a),!0):checkForSmashTurn(a)?(aS[3].init(a),!0):.3<Math.abs(player[a].inputs.lStickAxis[0].x)?(aS[7].init(a,!0),!0):!1}},{name:"JUMPAERIALF",canPassThrough:!0,canGrabLedge:[!0,!1],wallJumpAble:!0,init:function(a){player[a].actionState=19;player[a].timer=0;player[a].phys.fastfalled=!1;player[a].phys.doubleJumped=!0;player[a].phys.cVel.y=player[a].charAttributes.fHopInitV*player[a].charAttributes.djMultiplier;player[a].phys.cVel.x=1*player[a].inputs.lStickAxis[0].x;drawVfx("doubleJumpRings",
player[a].phys.pos,player[a].phys.face);playSfx("jump2");aS[19].main(a)},main:function(a){player[a].timer++;aS[19].interrupt(a)||(fastfall(a),airDrift(a))},interrupt:function(a){var b=checkForAerials(a),d=checkForSpecials(a);return b[0]?(aS[b[1]].init(a),!0):player[a].inputs.l[0]&&!player[a].inputs.l[1]||player[a].inputs.r[0]&&!player[a].inputs.r[1]?(aS[11].init(a),!0):d[0]?(aS[d[1]].init(a),!0):50<player[a].timer?(aS[32].init(a),!0):!1}},{name:"PASS",canPassThrough:!0,canGrabLedge:[!0,!1],wallJumpAble:!0,
init:function(a){player[a].actionState=20;player[a].timer=0;player[a].phys.grounded=!1;player[a].phys.passFastfall=!1;player[a].phys.abovePlatforms[player[a].phys.onSurface]=!1;player[a].phys.cVel.y=-.5;aS[20].main(a)},main:function(a){player[a].timer++;1<player[a].timer&&!aS[20].interrupt(a)&&(player[a].phys.passFastfall?fastfall(a):(player[a].phys.cVel.y-=player[a].charAttributes.gravity,player[a].phys.cVel.y<-player[a].charAttributes.terminalV&&(player[a].phys.cVel.y=-player[a].charAttributes.terminalV),
-.3<player[a].inputs.lStickAxis[0].y&&(player[a].phys.passFastfall=!0)),airDrift(a))},interrupt:function(a){var b=checkForAerials(a),d=checkForSpecials(a);return b[0]?(aS[b[1]].init(a),!0):player[a].inputs.l[0]&&!player[a].inputs.l[1]||player[a].inputs.r[0]&&!player[a].inputs.r[1]?(aS[11].init(a),!0):(player[a].inputs.x[0]&&!player[a].inputs.x[1]||player[a].inputs.y[0]&&!player[a].inputs.y[1]||.7<player[a].inputs.lStickAxis[0].y&&.7>=player[a].inputs.lStickAxis[1].y)&&!player[a].phys.doubleJumped?
(aS[19].init(a),!0):d[0]?(aS[d[1]].init(a),!0):30<player[a].timer?(aS[13].init(a),!0):!1}},{name:"GUARDON",canEdgeCancel:!0,init:function(a){player[a].actionState=21;player[a].timer=0;player[a].phys.shielding=!0;player[a].phys.shieldPosition=new Vec2D(0,0);player[a].phys.powerShielded=!1;shieldSize(a,!0);player[a].phys.powerShieldActive=1==Math.max(player[a].inputs.lAnalog[0],player[a].inputs.rAnalog[0])?!0:!1;aS[21].main(a)},main:function(a){0<player[a].hit.shieldstun?reduceByTraction(a,!1):(player[a].timer++,
2==player[a].timer&&1==Math.max(player[a].inputs.lAnalog[0],player[a].inputs.rAnalog[0])&&(player[a].phys.powerShieldActive=!0),aS[21].interrupt(a)||(1==player[a].timer&&(playSfx("shieldup"),playSfx("swordsheath")),reduceByTraction(a,!1),0>=player[a].hit.shieldstun&&(shieldDepletion(a),shieldSize(a,!0),shieldTilt(a))))},interrupt:function(a){return player[a].inCSS?8<player[a].timer?(aS[22].init(a),!0):!1:checkForJump(a)||.6<player[a].inputs.cStickAxis[0].y&&.3>player[a].inputs.cStickAxis[3].y?(player[a].phys.shielding=
...
player[a].charHitboxes.upb1.id2;player[a].phys.landingMultiplier=30/34;playSfx("dolphinSlash");playSfx("dolphinSlash2");aS[38].main(a)},main:function(a){player[a].timer++;aS[38].interrupt(a)||(0>=player[a].phys.cVel.y&&(player[a].phys.canWallJump=!0),6>player[a].timer&&.7<Math.abs(player[a].inputs.lStickAxis[0].x)&&(player[a].phys.upbAngleMultiplier=-player[a].inputs.lStickAxis[0].x*Math.PI/16),6==player[a].timer&&-.7>player[a].inputs.lStickAxis[0].x*player[a].phys.face&&(player[a].phys.face*=-1),
5<player[a].timer&&23>player[a].timer?player[a].phys.cVel=new Vec2D(aS[38].setVelocities[player[a].timer-6][0]*player[a].phys.face*Math.cos(player[a].phys.upbAngleMultiplier)-aS[38].setVelocities[player[a].timer-6][1]*Math.sin(player[a].phys.upbAngleMultiplier),aS[38].setVelocities[player[a].timer-6][0]*player[a].phys.face*Math.sin(player[a].phys.upbAngleMultiplier)+aS[38].setVelocities[player[a].timer-6][1]*Math.cos(player[a].phys.upbAngleMultiplier)):22<player[a].timer&&(fastfall(a),airDrift(a)),
5==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!1],player[a].hitboxes.frame=0),6==player[a].timer&&(player[a].hitboxes.id0=player[a].charHitboxes.upb2.id0,player[a].hitboxes.id1=player[a].charHitboxes.upb2.id1,player[a].hitboxes.id2=player[a].charHitboxes.upb2.id2),6<player[a].timer&&11>player[a].timer&&player[a].hitboxes.frame++,11==player[a].timer&&turnOffHitboxes(a),2<player[a].timer&&12>player[a].timer&&drawVfx("swingUpb",new Vec2D(player[a].phys.pos.x+player[a].phys.cVel.x,player[a].phys.pos.y+
player[a].phys.cVel.y),player[a].phys.face,player[a].timer-3))},interrupt:function(a){return 39<player[a].timer?(aS[14].init(a),!0):!1}},{name:"DAMAGEFLYN",canPassThrough:!1,canGrabLedge:[!1,!1],wallJumpAble:!1,init:function(a,b){player[a].actionState=39;player[a].timer=0;player[a].phys.grabbing=-1;player[a].phys.grabbedBy=-1;b&&(drawVfx("hitSparks",player[a].hit.hitPoint,player[a].phys.face),drawVfx("hitFlair",player[a].hit.hitPoint,player[a].phys.face),drawVfx("hitCurve",player[a].hit.hitPoint,
player[a].phys.face,player[a].hit.angle));player[a].hitboxes.id0=player[a].charHitboxes.thrown.id0;turnOffHitboxes(a);aS[39].main(a)},main:function(a){player[a].phys.thrownHitbox&&(1==player[a].timer&&0<player[a].phys.cVel.y+player[a].phys.kVel.y&&(player[a].hitboxes.active=[!0,!1,!1,!1],player[a].hitboxes.frame=0),1<player[a].timer&&0<player[a].phys.cVel.y+player[a].phys.kVel.y&&player[a].hitboxes.frame++,0>=player[a].phys.cVel.y+player[a].phys.kVel.y&&turnOffHitboxes(a));29>player[a].timer&&player[a].timer++;
0==player[a].hit.hitstun%10&&drawVfx("flyingDust",player[a].phys.pos);aS[39].interrupt(a)?player[a].phys.thrownHitbox=!1:1<player[a].timer&&(player[a].hit.hitstun--,player[a].phys.grounded||(player[a].phys.cVel.y-=player[a].charAttributes.gravity,player[a].phys.cVel.y<-player[a].charAttributes.terminalV&&(player[a].phys.cVel.y=-player[a].charAttributes.terminalV)))},interrupt:function(a){return 1<player[a].timer&&0==player[a].hit.hitstun?(aS[40].init(a),!0):!1}},{name:"DAMAGEFALL",canPassThrough:!1,
canGrabLedge:[!0,!1],wallJumpAble:!1,init:function(a){player[a].actionState=40;player[a].timer=0;turnOffHitboxes(a);aS[40].main(a)},main:function(a){player[a].timer++;aS[40].interrupt(a)||(fastfall(a),airDrift(a))},interrupt:function(a){var b=checkForAerials(a),d=checkForSpecials(a);return b[0]?(aS[b[1]].init(a),!0):player[a].inputs.l[0]&&!player[a].inputs.l[1]||player[a].inputs.r[0]&&!player[a].inputs.r[1]?(aS[11].init(a),!0):(player[a].inputs.x[0]&&!player[a].inputs.x[1]||player[a].inputs.y[0]&&
!player[a].inputs.y[1]||.7<player[a].inputs.lStickAxis[0].y&&.7>=player[a].inputs.lStickAxis[1].y)&&!player[a].phys.doubleJumped?(aS[19].init(a),!0):d[0]?(aS[d[1]].init(a),!0):.7<player[a].inputs.lStickAxis[0].x&&.7>player[a].inputs.lStickAxis[1].x||-.7>player[a].inputs.lStickAxis[0].x&&-.7<player[a].inputs.lStickAxis[1].x||.7<player[a].inputs.lStickAxis[0].y&&.7>player[a].inputs.lStickAxis[1].y||-.7>player[a].inputs.lStickAxis[0].y&&-.7<player[a].inputs.lStickAxis[1].y?(aS[13].init(a),!0):30<player[a].timer?
(aS[40].init(a),!0):!1}},{name:"DAMAGEN2",canEdgeCancel:!0,canPassThrough:!1,canGrabLedge:[!1,!1],wallJumpAble:!1,init:function(a){player[a].actionState=41;player[a].timer=0;player[a].phys.grabbing=-1;player[a].phys.grabbedBy=-1;turnOffHitboxes(a);drawVfx("hitSparks",player[a].hit.hitPoint,player[a].phys.face);drawVfx("hitFlair",player[a].hit.hitPoint,player[a].phys.face);drawVfx("hitCurve",player[a].hit.hitPoint,player[a].phys.face,player[a].hit.angle);aS[41].main(a)},main:function(a){player[a].timer++;
!aS[41].interrupt(a)&&1<player[a].timer&&(player[a].hit.hitstun--,player[a].phys.grounded?reduceByTraction(a,!1):(player[a].phys.cVel.y-=player[a].charAttributes.gravity,player[a].phys.cVel.y<-player[a].charAttributes.terminalV&&(player[a].phys.cVel.y=-player[a].charAttributes.terminalV)))},interrupt:function(a){if(23<player[a].timer)return player[a].phys.grounded?aS[0].init(a):aS[13].init(a),!0;if(0>=player[a].hit.hitstun){if(player[a].phys.grounded){var b=checkForSpecials(a),d=checkForTilts(a),
e=checkForSmashes(a);return checkForJump(a)?(aS[8].init(a),!0):player[a].inputs.l[0]||player[a].inputs.r[0]||0<player[a].inputs.lAnalog[0]||0<player[a].inputs.rAnalog[0]?(aS[21].init(a),!0):b[0]?(aS[b[1]].init(a),!0):e[0]?(aS[e[1]].init(a),!0):d[0]?(aS[d[1]].init(a),!0):checkForSquat(a)?(aS[16].init(a),!0):checkForDash(a)?(aS[1].init(a),!0):checkForSmashTurn(a)?(aS[3].init(a),!0):checkForTiltTurn(a)?(player[a].phys.dashbuffer=tiltTurnDashBuffer(a),aS[4].init(a),!0):.3<Math.abs(player[a].inputs.lStickAxis[0].x)?
(aS[7].init(a,!0),!0):!1}d=checkForAerials(a);b=checkForSpecials(a);if(d[0])return aS[d[1]].init(a),!0;if(player[a].inputs.l[0]&&!player[a].inputs.l[1]||player[a].inputs.r[0]&&!player[a].inputs.r[1])return aS[11].init(a),!0;if((player[a].inputs.x[0]&&!player[a].inputs.x[1]||player[a].inputs.y[0]&&!player[a].inputs.y[1]||.7<player[a].inputs.lStickAxis[0].y&&.7>=player[a].inputs.lStickAxis[1].y)&&!player[a].phys.doubleJumped)return aS[19].init(a),!0;if(b[0])return aS[b[1]].init(a),!0;if(.7<player[a].inputs.lStickAxis[0].x&&
.7>player[a].inputs.lStickAxis[1].x||-.7>player[a].inputs.lStickAxis[0].x&&-.7<player[a].inputs.lStickAxis[1].x||.7<player[a].inputs.lStickAxis[0].y&&.7>player[a].inputs.lStickAxis[1].y||-.7>player[a].inputs.lStickAxis[0].y&&-.7<player[a].inputs.lStickAxis[1].y)return aS[13].init(a),!0;if(30<player[a].timer)return aS[40].init(a),!0}return!1}},{name:"LANDINGATTACKAIRN",canEdgeCancel:!0,init:function(a){player[a].actionState=42;player[a].timer=0;player[a].phys.landingLagScaling=player[a].phys.lCancel?
2:1;drawVfx("circleDust",player[a].phys.pos,player[a].phys.face);playSfx("land");aS[42].main(a)},main:function(a){player[a].timer+=2*player[a].phys.landingLagScaling;aS[42].interrupt(a)||reduceByTraction(a,!0)},interrupt:function(a){return 30<player[a].timer?(aS[0].init(a),!0):!1}},{name:"LANDINGATTACKAIRF",canEdgeCancel:!0,init:function(a){player[a].actionState=43;player[a].timer=0;player[a].phys.landingLagScaling=player[a].phys.lCancel?2:1;drawVfx("circleDust",player[a].phys.pos,player[a].phys.face);
playSfx("land");aS[43].main(a)},main:function(a){player[a].timer+=2*player[a].phys.landingLagScaling;aS[43].interrupt(a)||reduceByTraction(a,!0)},interrupt:function(a){return 30<player[a].timer?(aS[0].init(a),!0):!1}},{name:"LANDINGATTACKAIRB",canEdgeCancel:!0,init:function(a){player[a].actionState=44;player[a].timer=0;player[a].phys.landingLagScaling=player[a].phys.lCancel?2:1;drawVfx("circleDust",player[a].phys.pos,player[a].phys.face);playSfx("land");aS[44].main(a)},main:function(a){player[a].timer+=
1.25*player[a].phys.landingLagScaling;aS[44].interrupt(a)||reduceByTraction(a,!0)},interrupt:function(a){return 30<player[a].timer?(aS[0].init(a),!0):!1}},{name:"LANDINGATTACKAIRD",canEdgeCancel:!0,init:function(a){player[a].actionState=45;player[a].timer=0;player[a].phys.landingLagScaling=player[a].phys.lCancel?2:1;drawVfx("circleDust",player[a].phys.pos,player[a].phys.face);playSfx("land");aS[45].main(a)},main:function(a){player[a].timer+=.9375*player[a].phys.landingLagScaling;aS[45].interrupt(a)||
reduceByTraction(a,!0)},interrupt:function(a){return 30<player[a].timer?(aS[0].init(a),!0):!1}},{name:"LANDINGATTACKAIRU",canEdgeCancel:!0,init:function(a){player[a].actionState=46;player[a].timer=0;player[a].phys.landingLagScaling=player[a].phys.lCancel?2:1;drawVfx("circleDust",player[a].phys.pos,player[a].phys.face);playSfx("land");aS[46].main(a)},main:function(a){player[a].timer+=2*player[a].phys.landingLagScaling;aS[46].interrupt(a)||reduceByTraction(a,!0)},interrupt:function(a){return 30<player[a].timer?
(aS[0].init(a),!0):!1}},{name:"DOWNTILT",canEdgeCancel:!1,init:function(a){player[a].actionState=47;player[a].timer=0;turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.dtilt.id0;player[a].hitboxes.id1=player[a].charHitboxes.dtilt.id1;player[a].hitboxes.id2=player[a].charHitboxes.dtilt.id2;player[a].hitboxes.id3=player[a].charHitboxes.dtilt.id3;aS[47].main(a)},main:function(a){player[a].timer++;aS[47].interrupt(a)||(reduceByTraction(a,!0),5<player[a].timer&&10>player[a].timer&&drawVfx("swingDtilt",
new Vec2D(player[a].phys.pos.x+player[a].phys.cVel.x,player[a].phys.pos.y+player[a].phys.cVel.y),player[a].phys.face,player[a].timer-6),7==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,playSfx("sword2")),7<player[a].timer&&10>player[a].timer&&player[a].hitboxes.frame++,10==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){if(49<player[a].timer)return aS[17].init(a),!0;if(19<player[a].timer){var b=checkForSpecials(a),d=checkForTilts(a),e=checkForSmashes(a);
if(checkForJump(a))return aS[8].init(a),!0;if(b[0])return aS[b[1]].init(a),!0;if(e[0])return aS[e[1]].init(a),!0;if(d[0])return aS[d[1]].init(a),!0;if(checkForDash(a))return aS[1].init(a),!0;if(checkForSmashTurn(a))return aS[3].init(a),!0;if(-.3>player[a].inputs.lStickAxis[0].x*player[a].phys.face&&Math.abs(player[a].inputs.lStickAxis[0].x)>-1*player[a].inputs.lStickAxis[0].y)return player[a].phys.dashbuffer=tiltTurnDashBuffer(a),aS[4].init(a),!0;if(.3<player[a].inputs.lStickAxis[0].x*player[a].phys.face&&
Math.abs(player[a].inputs.lStickAxis[0].x)>-1*player[a].inputs.lStickAxis[0].y)return aS[7].init(a,!0),!0}return!1}},{name:"UPTILT",canEdgeCancel:!1,init:function(a){player[a].actionState=48;player[a].timer=0;turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.uptilt1.id0;player[a].hitboxes.id1=player[a].charHitboxes.uptilt1.id1;player[a].hitboxes.id2=player[a].charHitboxes.uptilt1.id2;player[a].hitboxes.id3=player[a].charHitboxes.uptilt1.id3;aS[48].main(a)},main:function(a){player[a].timer++;
aS[48].interrupt(a)||(reduceByTraction(a,!0),4<player[a].timer&&15>player[a].timer&&drawVfx("swingUptilt",new Vec2D(player[a].phys.pos.x+player[a].phys.cVel.x,player[a].phys.pos.y+player[a].phys.cVel.y),player[a].phys.face,player[a].timer-5),6==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,playSfx("sword2")),6<player[a].timer&&13>player[a].timer&&player[a].hitboxes.frame++,9==player[a].timer&&(player[a].hitboxes.frame=0,player[a].hitboxes.id0=player[a].charHitboxes.uptilt2.id0,
player[a].hitboxes.id1=player[a].charHitboxes.uptilt2.id1,player[a].hitboxes.id2=player[a].charHitboxes.uptilt2.id2,player[a].hitboxes.id3=player[a].charHitboxes.uptilt2.id3),13==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){if(39<player[a].timer)return aS[0].init(a),!0;if(31<player[a].timer){var b=checkForSpecials(a),d=checkForTilts(a),e=checkForSmashes(a);if(checkForJump(a))return aS[8].init(a),!0;if(b[0])return aS[b[1]].init(a),!0;if(e[0])return aS[e[1]].init(a),!0;if(d[0])return aS[d[1]].init(a),
!0;if(checkForDash(a))return aS[1].init(a),!0;if(checkForSmashTurn(a))return aS[3].init(a),!0;if(checkForTiltTurn(a))return player[a].phys.dashbuffer=tiltTurnDashBuffer(a),aS[4].init(a),!0;if(.3<Math.abs(player[a].inputs.lStickAxis[0].x))return aS[7].init(a,!0),!0}return!1}},{name:"FTILT",canEdgeCancel:!1,init:function(a){player[a].actionState=49;player[a].timer=0;turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.ftilt.id0;player[a].hitboxes.id1=player[a].charHitboxes.ftilt.id1;player[a].hitboxes.id2=
player[a].charHitboxes.ftilt.id2;player[a].hitboxes.id3=player[a].charHitboxes.ftilt.id3;aS[49].main(a)},main:function(a){player[a].timer++;aS[49].interrupt(a)||(reduceByTraction(a,!0),7==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,playSfx("sword2")),7<player[a].timer&&11>player[a].timer&&player[a].hitboxes.frame++,11==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){return 35<player[a].timer?(aS[0].init(a),!0):!1}},{name:"JAB1",canEdgeCancel:!1,
init:function(a){player[a].actionState=50;player[a].timer=0;player[a].phys.jabCombo=!1;turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.jab1.id0;player[a].hitboxes.id1=player[a].charHitboxes.jab1.id1;player[a].hitboxes.id2=player[a].charHitboxes.jab1.id2;player[a].hitboxes.id3=player[a].charHitboxes.jab1.id3;aS[50].main(a)},main:function(a){player[a].timer++;aS[50].interrupt(a)||(reduceByTraction(a,!0),2<player[a].timer&&26>player[a].timer&&player[a].inputs.a[0]&&!player[a].inputs.a[1]&&
(player[a].phys.jabCombo=!0),4==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,playSfx("sword1")),4<player[a].timer&&8>player[a].timer&&player[a].hitboxes.frame++,8==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){if(19<player[a].timer&&player[a].phys.jabCombo)return aS[51].init(a),!0;if(27<player[a].timer)return aS[0].init(a),!0;if(26<player[a].timer){var b=checkForSpecials(a),d=checkForTilts(a),e=checkForSmashes(a);if(checkForJump(a))return aS[8].init(a),
!0;if(b[0])return aS[b[1]].init(a),!0;if(e[0])return aS[e[1]].init(a),!0;if(d[0])return aS[d[1]].init(a),!0;if(checkForDash(a))return aS[1].init(a),!0;if(checkForSmashTurn(a))return aS[3].init(a),!0;if(checkForTiltTurn(a))return player[a].phys.dashbuffer=tiltTurnDashBuffer(a),aS[4].init(a),!0;if(.3<Math.abs(player[a].inputs.lStickAxis[0].x))return aS[7].init(a,!0),!0}return!1}},{name:"JAB2",canEdgeCancel:!1,init:function(a){player[a].actionState=51;player[a].timer=0;player[a].phys.jabCombo=!1;turnOffHitboxes(a);
player[a].hitboxes.id0=player[a].charHitboxes.jab2.id0;player[a].hitboxes.id1=player[a].charHitboxes.jab2.id1;player[a].hitboxes.id2=player[a].charHitboxes.jab2.id2;player[a].hitboxes.id3=player[a].charHitboxes.jab2.id3;aS[51].main(a)},main:function(a){player[a].timer++;aS[51].interrupt(a)||(reduceByTraction(a,!0),1<player[a].timer&&26>player[a].timer&&player[a].inputs.a[0]&&!player[a].inputs.a[1]&&(player[a].phys.jabCombo=!0),5==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=
0,playSfx("sword1")),5<player[a].timer&&10>player[a].timer&&player[a].hitboxes.frame++,10==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){if(19<player[a].timer&&player[a].phys.jabCombo)return aS[50].init(a),!0;if(28<player[a].timer)return aS[0].init(a),!0;if(27<player[a].timer){var b=checkForSpecials(a),d=checkForTilts(a),e=checkForSmashes(a);if(checkForJump(a))return aS[8].init(a),!0;if(b[0])return aS[b[1]].init(a),!0;if(e[0])return aS[e[1]].init(a),!0;if(d[0])return aS[d[1]].init(a),
!0;if(checkForDash(a))return aS[1].init(a),!0;if(checkForSmashTurn(a))return aS[3].init(a),!0;if(checkForTiltTurn(a))return player[a].phys.dashbuffer=tiltTurnDashBuffer(a),aS[4].init(a),!0;if(.3<Math.abs(player[a].inputs.lStickAxis[0].x))return aS[7].init(a,!0),!0}return!1}},{name:"ESCAPEB",setVelocities:[-2.267,-2.536,-2.706,-2.78,-2.758,-2.64,-2.426,-2.116,-1.711,-1.209,-.888,-.819,-.758,-.707,-.664,-.631,-.606,-.591,-.585,-.587,-.599,-.62,-.809,-1.072,-1.205,-1.207,-1.078,-.819,-.617,-.556,-.487,
-.413,-.332,-.245,-.152],canEdgeCancel:!1,init:function(a){player[a].actionState=52;player[a].timer=0;player[a].phys.shielding=!1;aS[52].main(a)},main:function(a){player[a].timer++;aS[52].interrupt(a)||(33==player[a].timer&&playSfx("sworddraw"),player[a].phys.cVel.x=aS[52].setVelocities[player[a].timer-1]*player[a].phys.face,4==player[a].timer&&(player[a].phys.intangibleTimer=20,playSfx("roll")))},interrupt:function(a){return 35<player[a].timer?(player[a].phys.cVel.x=0,aS[0].init(a),!0):!1}},{name:"ESCAPEF",
setVelocities:[2.267,2.536,2.706,2.78,2.758,2.64,2.426,2.116,1.711,1.209,.888,.819,.758,.707,.664,.631,.606,.591,.585,.587,.599,.62,.809,1.072,1.205,1.207,1.078,.819,.617,.556,.487,.413,.332,.245,.152],canEdgeCancel:!1,reverseModel:!0,init:function(a){player[a].actionState=53;player[a].timer=0;player[a].phys.shielding=!1;aS[53].main(a)},main:function(a){player[a].timer++;aS[53].interrupt(a)||(33==player[a].timer&&playSfx("sworddraw"),player[a].phys.cVel.x=aS[53].setVelocities[player[a].timer-1]*player[a].phys.face,
4==player[a].timer&&(player[a].phys.intangibleTimer=20,playSfx("roll")))},interrupt:function(a){return 35<player[a].timer?(player[a].phys.cVel.x=0,player[a].phys.face*=-1,aS[0].init(a),!0):!1}},{name:"ESCAPEN",canEdgeCancel:!1,init:function(a){player[a].actionState=54;player[a].timer=0;player[a].phys.shielding=!1;drawVfx("circleDust",player[a].phys.pos,player[a].phys.face);aS[54].main(a)},main:function(a){player[a].timer++;aS[54].interrupt(a)||(1==player[a].timer&&playSfx("spotdodge"),2==player[a].timer&&
playSfx("airdodge"),18==player[a].timer&&playSfx("sworddraw"),reduceByTraction(a,!0),2==player[a].timer&&(player[a].phys.intangibleTimer=16))},interrupt:function(a){return 27<player[a].timer?(aS[0].init(a),!0):!1}},{name:"ATTACKDASH",canEdgeCancel:!1,setVelocities:[.755,1.962,2.714,3.01,2.849,2.232,1.184,.542,.704,1.325,1.487,1.079,.666,.631,.597,.565,.536,.508,.482,.458,.436,.416,.398,.37,.332,.299,.27,.244,.222,.205,.191,.181,.176,.165,.148,.13,.112,.093,.073,.053,.032,.011,-.783,-.783,0,0,.001,
.001,0],init:function(a){player[a].actionState=55;player[a].timer=0;turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.dashattack.id0;player[a].hitboxes.id1=player[a].charHitboxes.dashattack.id1;player[a].hitboxes.id2=player[a].charHitboxes.dashattack.id2;player[a].hitboxes.id3=player[a].charHitboxes.dashattack.id3;aS[55].main(a)},main:function(a){player[a].timer++;aS[55].interrupt(a)||(player[a].phys.cVel.x=aS[55].setVelocities[player[a].timer-1]*player[a].phys.face,12==player[a].timer&&
(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,playSfx("sword1")),12<player[a].timer&&16>player[a].timer&&player[a].hitboxes.frame++,16==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){if(49<player[a].timer)return aS[0].init(a),!0;if(5>player[a].timer&&0<player[a].inputs.lAnalog[0]||0<player[a].inputs.rAnalog[0])return player[a].phys.cVel.x*player[a].phys.face>player[a].charAttributes.dMaxV&&(player[a].phys.cVel.x=player[a].charAttributes.dMaxV*player[a].phys.face),
aS[73].init(a),!0;if(39<player[a].timer){var b=checkForSpecials(a),d=checkForTilts(a),e=checkForSmashes(a);if(checkForJump(a))return aS[8].init(a),!0;if(b[0])return aS[b[1]].init(a),!0;if(e[0])return aS[e[1]].init(a),!0;if(d[0])return aS[d[1]].init(a),!0;if(checkForDash(a))return aS[1].init(a),!0;if(checkForSmashTurn(a))return aS[3].init(a),!0;if(checkForTiltTurn(a))return player[a].phys.dashbuffer=tiltTurnDashBuffer(a),aS[4].init(a),!0;if(.3<Math.abs(player[a].inputs.lStickAxis[0].x))return aS[7].init(a,
!0),!0}return!1}},{name:"SIDESPECIAL",canPassThrough:!1,canEdgeCancel:!1,canGrabLedge:[!1,!1],wallJumpAble:!1,init:function(a){player[a].actionState=56;player[a].timer=0;player[a].phys.grounded?player[a].phys.cVel.x*=.2:(player[a].phys.sideBJumpFlag?(player[a].phys.cVel.y=1,player[a].phys.sideBJumpFlag=!1):player[a].phys.cVel.y=0,player[a].phys.fastfalled=!1,player[a].phys.cVel.x*=.8);turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.sidespecial.id0;player[a].hitboxes.id1=player[a].charHitboxes.sidespecial.id1;
player[a].hitboxes.id2=player[a].charHitboxes.sidespecial.id2;player[a].hitboxes.id3=player[a].charHitboxes.sidespecial.id3;playSfx("shout6");aS[56].main(a)},main:function(a){player[a].timer++;aS[56].interrupt(a)||(6==player[a].timer&&playSfx("dancingBlade"),player[a].phys.grounded?reduceByTraction(a,!0):(player[a].phys.cVel.y-=.06,0<player[a].phys.cVel.x?(player[a].phys.cVel.x-=.0025,0>player[a].phys.cVel.x&&(player[a].phys.cVel.x=0)):(player[a].phys.cVel.x+=.0025,0<player[a].phys.cVel.x&&(player[a].phys.cVel.x=
0))),6==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0),6<player[a].timer&&9>player[a].timer&&player[a].hitboxes.frame++,9==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){return 29<player[a].timer?(player[a].phys.grounded?aS[0].init(a):aS[13].init(a),!0):!1}},{name:"FSMASH",canEdgeCancel:!1,init:function(a){player[a].actionState=57;player[a].timer=0;player[a].phys.charging=!1;player[a].phys.chargeFrames=0;turnOffHitboxes(a);player[a].hitboxes.id0=
player[a].charHitboxes.fsmash.id0;player[a].hitboxes.id1=player[a].charHitboxes.fsmash.id1;player[a].hitboxes.id2=player[a].charHitboxes.fsmash.id2;player[a].hitboxes.id3=player[a].charHitboxes.fsmash.id3;aS[57].main(a)},main:function(a){3==player[a].timer?player[a].inputs.a[0]||player[a].inputs.z[0]?(player[a].phys.charging=!0,player[a].phys.chargeFrames++,5==player[a].phys.chargeFrames&&playSfx("smashcharge"),60==player[a].phys.chargeFrames&&(player[a].timer++,player[a].phys.charging=!1)):(player[a].timer++,
player[a].phys.charging=!1):(player[a].timer++,player[a].phys.charging=!1);aS[57].interrupt(a)||(reduceByTraction(a,!0),5==player[a].timer&&playSfx("sword3"),10==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,randomShout()),10<player[a].timer&&14>player[a].timer&&player[a].hitboxes.frame++,14==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){if(49<player[a].timer)return aS[0].init(a),!0;if(47<player[a].timer&&!player[a].inCSS){var b=checkForSpecials(a),
d=checkForTilts(a),e=checkForSmashes(a);if(checkForJump(a))return aS[8].init(a),!0;if(b[0])return aS[b[1]].init(a),!0;if(e[0])return aS[e[1]].init(a),!0;if(d[0])return aS[d[1]].init(a),!0;if(checkForDash(a))return aS[1].init(a),!0;if(checkForSmashTurn(a))return aS[3].init(a),!0;if(checkForTiltTurn(a))return player[a].phys.dashbuffer=tiltTurnDashBuffer(a),aS[4].init(a),!0;if(.3<Math.abs(player[a].inputs.lStickAxis[0].x))return aS[7].init(a,!0),!0}return!1}},{name:"UPSMASH",canEdgeCancel:!1,init:function(a){player[a].actionState=
58;player[a].timer=0;player[a].phys.charging=!1;player[a].phys.chargeFrames=0;turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.upsmash.id0;player[a].hitboxes.id1=player[a].charHitboxes.upsmash.id1;player[a].hitboxes.id2=player[a].charHitboxes.upsmash.id2;player[a].hitboxes.id3=player[a].charHitboxes.upsmash.id3;aS[58].main(a)},main:function(a){7==player[a].timer?player[a].inputs.a[0]||player[a].inputs.z[0]?(player[a].phys.charging=!0,player[a].phys.chargeFrames++,5==player[a].phys.chargeFrames&&
playSfx("smashcharge"),60==player[a].phys.chargeFrames&&(player[a].timer++,player[a].phys.charging=!1)):(player[a].timer++,player[a].phys.charging=!1):(player[a].timer++,player[a].phys.charging=!1);aS[58].interrupt(a)||(reduceByTraction(a,!0),13==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,playSfx("sword3"),randomShout()),13<player[a].timer&&17>player[a].timer&&player[a].hitboxes.frame++,17==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){if(54<
player[a].timer)return aS[0].init(a),!0;if(45<player[a].timer&&!player[a].inCSS){var b=checkForSpecials(a),d=checkForTilts(a),e=checkForSmashes(a);if(checkForJump(a))return aS[8].init(a),!0;if(b[0])return aS[b[1]].init(a),!0;if(e[0])return aS[e[1]].init(a),!0;if(d[0])return aS[d[1]].init(a),!0;if(checkForDash(a))return aS[1].init(a),!0;if(checkForSmashTurn(a))return aS[3].init(a),!0;if(checkForTiltTurn(a))return player[a].phys.dashbuffer=tiltTurnDashBuffer(a),aS[4].init(a),!0;if(.3<Math.abs(player[a].inputs.lStickAxis[0].x))return aS[7].init(a,
!0),!0}return!1}},{name:"DSMASH",canEdgeCancel:!1,init:function(a){player[a].actionState=59;player[a].timer=0;player[a].phys.charging=!1;player[a].phys.chargeFrames=0;turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.dsmash1.id0;player[a].hitboxes.id1=player[a].charHitboxes.dsmash1.id1;player[a].hitboxes.id2=player[a].charHitboxes.dsmash1.id2;player[a].hitboxes.id3=player[a].charHitboxes.dsmash1.id3;aS[59].main(a)},main:function(a){3==player[a].timer?player[a].inputs.a[0]||player[a].inputs.z[0]?
(player[a].phys.charging=!0,player[a].phys.chargeFrames++,5==player[a].phys.chargeFrames&&playSfx("smashcharge"),60==player[a].phys.chargeFrames&&(player[a].timer++,player[a].phys.charging=!1)):(player[a].timer++,player[a].phys.charging=!1):(player[a].timer++,player[a].phys.charging=!1);aS[59].interrupt(a)||(reduceByTraction(a,!0),5==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,playSfx("sword3"),randomShout()),5<player[a].timer&&8>player[a].timer&&player[a].hitboxes.frame++,
...
(aS[0].init(a),!0):!1}},{name:"DOWNATTACK",canEdgeCancel:!1,init:function(a){player[a].actionState=65;player[a].timer=0;turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.downattack1.id0;player[a].hitboxes.id1=player[a].charHitboxes.downattack1.id1;player[a].hitboxes.id2=player[a].charHitboxes.downattack1.id2;player[a].hitboxes.id3=player[a].charHitboxes.downattack1.id3;aS[65].main(a)},main:function(a){player[a].timer++;aS[65].interrupt(a)||(reduceByTraction(a,!0),1==player[a].timer&&
(player[a].phys.intangibleTimer=31),20==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,playSfx("sword2")),20<player[a].timer&&24>player[a].timer&&player[a].hitboxes.frame++,24==player[a].timer&&turnOffHitboxes(a),30==player[a].timer&&(player[a].hitboxes.id0=player[a].charHitboxes.downattack2.id0,player[a].hitboxes.id1=player[a].charHitboxes.downattack2.id1,player[a].hitboxes.id2=player[a].charHitboxes.downattack2.id2,player[a].hitboxes.id3=player[a].charHitboxes.downattack2.id3,
player[a].hitboxes.active=[!0,!0,!0,!0],player[a].hitboxes.frame=0,playSfx("sword2")),30<player[a].timer&&32>player[a].timer&&player[a].hitboxes.frame++,32==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){return 49<player[a].timer?(aS[0].init(a),!0):!1}},{name:"TECHN",canEdgeCancel:!0,init:function(a){player[a].actionState=66;player[a].timer=0;drawVfx("tech",player[a].phys.pos);playSfx("tech");aS[66].main(a)},main:function(a){player[a].timer++;aS[66].interrupt(a)||(reduceByTraction(a,
!0),1==player[a].timer&&(player[a].phys.intangibleTimer=20))},interrupt:function(a){return 26<player[a].timer?(aS[0].init(a),!0):!1}},{name:"TECHB",canEdgeCancel:!1,setVelocities:[0,0,0,0,0,0,0,-2.832,-2.726,-2.622,-2.521,-2.422,-2.326,-2.233,-2.142,-2.054,-1.968,-1.885,-1.811,-1.748,-1.691,-1.639,-1.593,-1.553,-1.519,-1.49,-1.467,-1.45,-1.439,-1.433,-1.433,-.002,-.003,-.004,-.005,-.006,-.006,-.007,-.007,-.006],init:function(a){player[a].actionState=67;player[a].timer=0;drawVfx("tech",player[a].phys.pos);
playSfx("tech");aS[67].main(a)},main:function(a){player[a].timer++;aS[67].interrupt(a)||(1==player[a].timer&&(player[a].phys.intangibleTimer=20),player[a].phys.cVel.x=aS[67].setVelocities[player[a].timer-1]*player[a].phys.face)},interrupt:function(a){return 40<player[a].timer?(aS[0].init(a),!0):!1}},{name:"TECHF",canEdgeCancel:!1,setVelocities:[0,0,0,0,0,0,0,4.036,3.526,2.726,2.317,1.862,1.656,1.625,1.768,1.989,2.094,2.083,1.956,1.846,1.814,1.757,1.676,1.57,1.44,1.286,1.107,.949,.834,.727,.629,.54,
.459,.387,.323,.268,.222,.184,.155,.135],init:function(a){player[a].actionState=68;player[a].timer=0;drawVfx("tech",player[a].phys.pos);playSfx("tech");aS[68].main(a)},main:function(a){player[a].timer++;aS[68].interrupt(a)||(1==player[a].timer&&(player[a].phys.intangibleTimer=20),player[a].phys.cVel.x=aS[68].setVelocities[player[a].timer-1]*player[a].phys.face)},interrupt:function(a){return 40<player[a].timer?(aS[0].init(a),!0):!1}},{name:"SHIELDBREAKFALL",canPassThrough:!1,canGrabLedge:[!1,!1],wallJumpAble:!1,
init:function(a){player[a].actionState=69;player[a].timer=0;aS[69].main(a)},main:function(a){player[a].timer++;aS[69].interrupt(a)||(player[a].phys.intangibleTimer=1,player[a].phys.cVel.y-=player[a].charAttributes.gravity)},interrupt:function(a){return 30<player[a].timer?(aS[69].init(a),!0):!1}},{name:"SHIELDBREAKDOWNBOUND",canEdgeCancel:!0,init:function(a){player[a].actionState=70;player[a].timer=0;player[a].phys.cVel.y=0;player[a].phys.kVel.y=0;drawVfx("groundBounce",player[a].phys.pos,player[a].phys.face);
playSfx("bounce");aS[70].main(a)},main:function(a){player[a].timer++;aS[70].interrupt(a)||(player[a].phys.intangibleTimer=1,1==player[a].timer?reduceByTraction(a,!0):player[a].phys.cVel.x=0)},interrupt:function(a){return 26<player[a].timer?(aS[71].init(a),!0):!1}},{name:"SHIELDBREAKSTAND",canEdgeCancel:!0,init:function(a){player[a].actionState=71;player[a].timer=0;aS[71].main(a)},main:function(a){player[a].timer++;aS[71].interrupt(a)||(reduceByTraction(a,!0),player[a].phys.intangibleTimer=1)},interrupt:function(a){return 30<
player[a].timer?(aS[72].init(a),!0):!1}},{name:"FURAFURA",canEdgeCancel:!0,init:function(a){player[a].actionState=72;player[a].timer=0;player[a].phys.stuckTimer=490;drawVfx("furaFura",new Vec2D(player[a].phys.pos.x+(4+2*Math.random())*player[a].phys.face,player[a].phys.pos.y+11+3*Math.random()),player[a].phys.face);player[a].furaLoopID=sounds.furaloop.play();aS[72].main(a)},main:function(a){player[a].timer++;aS[72].interrupt(a)||(65==player[a].timer%100&&playSfx("furacry"),reduceByTraction(a,!0),
0==player[a].timer%49&&drawVfx("furaFura",new Vec2D(player[a].phys.pos.x+(3+2*Math.random())*player[a].phys.face,player[a].phys.pos.y+11+3*Math.random()),player[a].phys.face),20==player[a].timer%49&&drawVfx("furaFura",new Vec2D(player[a].phys.pos.x+(5+2*Math.random())*player[a].phys.face,player[a].phys.pos.y+8+3*Math.random()),player[a].phys.face),30<player[a].phys.shieldHP&&(player[a].phys.shieldHP=30),player[a].phys.stuckTimer--,mashOut(a)&&(player[a].phys.stuckTimer-=3))},interrupt:function(a){return 0>=
player[a].phys.stuckTimer?(sounds.furaloop.stop(player[a].furaLoopID),aS[0].init(a),!0):!1}},{name:"GRAB",canEdgeCancel:!1,init:function(a){player[a].actionState=73;player[a].timer=0;player[a].phys.charging=!1;player[a].phys.chargeFrames=0;turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.grab.id0;player[a].hitboxes.id1=player[a].charHitboxes.grab.id1;player[a].hitboxes.id2=player[a].charHitboxes.grab.id2;aS[73].main(a)},main:function(a){player[a].timer++;aS[73].interrupt(a)||(reduceByTraction(a,
!0),7==player[a].timer&&(player[a].hitboxes.active=[!0,!0,!0,!1],player[a].hitboxes.frame=0,playSfx("grab")),7<player[a].timer&&9>player[a].timer&&player[a].hitboxes.frame++,9==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){return 30<player[a].timer?(aS[0].init(a),!0):!1}},{name:"CAPTUREPULLED",canEdgeCancel:!1,init:function(a){player[a].actionState=74;player[a].timer=0;player[a].phys.grounded=!0;player[a].phys.face=-1*player[player[a].phys.grabbedBy].phys.face;player[a].phys.onSurface=
player[player[a].phys.grabbedBy].phys.onSurface;player[a].phys.stuckTimer=100+2*player[a].percent;playSfx("grabbed");aS[74].main(a)},main:function(a){player[a].timer++;aS[74].interrupt(a)||2!=player[a].timer||(player[a].phys.pos=new Vec2D(player[player[a].phys.grabbedBy].phys.pos.x+-16.41205*player[a].phys.face,player[player[a].phys.grabbedBy].phys.pos.y))},interrupt:function(a){return 2<player[a].timer?(aS[75].init(a),aS[76].init(player[a].phys.grabbedBy),drawVfx("tech",new Vec2D(player[a].phys.pos.x,
player[a].phys.pos.y+10)),!0):!1}},{name:"CAPTUREWAIT",canEdgeCancel:!1,init:function(a){player[a].actionState=75;player[a].timer=0;player[a].phys.pos=new Vec2D(player[player[a].phys.grabbedBy].phys.pos.x+-9.04298*player[a].phys.face,player[player[a].phys.grabbedBy].phys.pos.y);aS[75].main(a)},main:function(a){player[a].timer++;aS[75].interrupt(a)||(player[a].phys.stuckTimer--,mashOut(a)?(player[a].phys.stuckTimer-=3,player[a].phys.pos.x+=.5*Math.sign(Math.random()-.5)):player[a].phys.pos.x=player[player[a].phys.grabbedBy].phys.pos.x+
...
player[a].phys.face||.7<player[a].inputs.cStickAxis[0].x*player[a].phys.face?(aS[87].init(a),!0):1<player[a].timer?(aS[76].init(a),!0):!1}},{name:"CAPTURECUT",canEdgeCancel:!1,canGrabLedge:[!0,!1],wallJumpAble:!1,init:function(a){player[a].actionState=77;player[a].timer=0;player[a].phys.grabbedBy=-1;player[a].phys.cVel.x=-1*player[a].phys.face;aS[77].main(a)},main:function(a){player[a].timer++;aS[77].interrupt(a)||(2==player[a].timer&&(player[a].phys.grabTech=!1),reduceByTraction(a,!0))},interrupt:function(a){return 30<
player[a].timer?(aS[0].init(a),!0):!1}},{name:"CATCHCUT",canEdgeCancel:!1,canGrabLedge:[!0,!1],init:function(a){player[a].actionState=78;player[a].timer=0;player[a].phys.grabbing=-1;player[a].phys.cVel.x=-1*player[a].phys.face;aS[78].main(a)},main:function(a){player[a].timer++;aS[78].interrupt(a)||reduceByTraction(a,!0)},interrupt:function(a){return 29<player[a].timer?(aS[0].init(a),!0):!1}},{name:"CATCHATTACK",canEdgeCancel:!1,init:function(a){player[a].actionState=79;player[a].timer=0;turnOffHitboxes(a);
player[a].hitboxes.id0=player[a].charHitboxes.pummel.id0;aS[79].main(a)},main:function(a){player[a].timer++;aS[79].interrupt(a)||(6==player[a].timer&&(player[a].hitboxes.active=[!0,!1,!1,!1],player[a].hitboxes.frame=0),7==player[a].timer&&turnOffHitboxes(a))},interrupt:function(a){return 24<player[a].timer?(aS[76].init(a),!0):!1}},{name:"CAPTUREDAMAGE",canEdgeCancel:!1,setPositions:[9.478,9.478,9.478,9.478,9.478,9.478,9.478,9.478,9.478,9.306,8.92,8.516,8.29,8.293,8.41,8.593,8.792,8.959,9.043,9.068],
init:function(a){player[a].actionState=80;player[a].timer=0;aS[80].main(a)},main:function(a){player[a].timer++;aS[80].interrupt(a)||(player[a].phys.pos.x=player[player[a].phys.grabbedBy].phys.pos.x+-aS[80].setPositions[player[a].timer-1]*player[a].phys.face)},interrupt:function(a){return 20<player[a].timer?(aS[75].init(a),!0):!1}},{name:"THROWUP",canEdgeCancel:!1,init:function(a){player[a].actionState=81;player[a].timer=0;aS[82].init(player[a].phys.grabbing);turnOffHitboxes(a);player[a].hitboxes.id0=
player[a].charHitboxes.throwup.id0;randomShout();aS[81].main(a)},main:function(a){player[a].timer++;aS[81].interrupt(a)||11!=player[a].timer||(hitQueue.push([player[a].phys.grabbing,a,0,!1,!0,!1]),turnOffHitboxes(a))},interrupt:function(a){return 39<player[a].timer?(player[a].phys.grabbing=-1,aS[0].init(a),!0):11>player[a].timer&&player[player[a].phys.grabbing].phys.grabbedBy!=a?(console.log("test"),aS[78].init(a),!0):!1}},{name:"THROWNUP",canEdgeCancel:!1,canGrabLedge:[!1,!1],init:function(a){player[a].actionState=
82;player[a].timer=player[a].phys.grabbedBy<a?-1:0;player[a].phys.grounded=!1;player[a].phys.pos=new Vec2D(player[player[a].phys.grabbedBy].phys.pos.x,player[player[a].phys.grabbedBy].phys.pos.y);aS[82].main(a)},main:function(a){player[a].timer++;aS[82].interrupt(a)},interrupt:function(a){return!1}},{name:"THROWDOWN",canEdgeCancel:!1,init:function(a){player[a].actionState=83;player[a].timer=0;aS[84].init(player[a].phys.grabbing);turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.throwdown.id0;
randomShout();aS[83].main(a)},main:function(a){player[a].timer++;aS[83].interrupt(a)||13!=player[a].timer||(hitQueue.push([player[a].phys.grabbing,a,0,!1,!0,!0]),turnOffHitboxes(a))},interrupt:function(a){return 37<player[a].timer?(player[a].phys.grabbing=-1,aS[0].init(a),!0):13>player[a].timer&&player[player[a].phys.grabbing].phys.grabbedBy!=a?(console.log("test"),aS[78].init(a),!0):!1}},{name:"THROWNDOWN",canEdgeCancel:!1,reverseModel:!0,canGrabLedge:[!1,!1],init:function(a){player[a].actionState=
84;player[a].timer=player[a].phys.grabbedBy<a?-1:0;player[a].phys.grounded=!1;player[a].phys.face*=-1;player[a].phys.pos=new Vec2D(player[player[a].phys.grabbedBy].phys.pos.x,player[player[a].phys.grabbedBy].phys.pos.y);aS[84].main(a)},main:function(a){player[a].timer++;aS[84].interrupt(a)},interrupt:function(a){return!1}},{name:"THROWBACK",canEdgeCancel:!1,init:function(a){player[a].actionState=85;player[a].timer=0;aS[86].init(player[a].phys.grabbing);turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.throwback.id0;
randomShout();aS[85].main(a)},main:function(a){player[a].timer++;aS[85].interrupt(a)||7!=player[a].timer||(hitQueue.push([player[a].phys.grabbing,a,0,!1,!0,!0]),turnOffHitboxes(a))},interrupt:function(a){return 39<player[a].timer?(player[a].phys.grabbing=-1,aS[0].init(a),!0):7>player[a].timer&&player[player[a].phys.grabbing].phys.grabbedBy!=a?(aS[78].init(a),!0):!1}},{name:"THROWNBACK",canEdgeCancel:!1,reverseModel:!0,canGrabLedge:[!1,!1],init:function(a){player[a].actionState=86;player[a].timer=
player[a].phys.grabbedBy<a?-1:0;player[a].phys.grounded=!1;player[a].phys.face*=-1;player[a].phys.pos=new Vec2D(player[player[a].phys.grabbedBy].phys.pos.x,player[player[a].phys.grabbedBy].phys.pos.y);aS[86].main(a)},main:function(a){player[a].timer++;aS[86].interrupt(a)},interrupt:function(a){return!1}},{name:"THROWFORWARD",canEdgeCancel:!1,init:function(a){player[a].actionState=87;player[a].timer=0;aS[88].init(player[a].phys.grabbing);turnOffHitboxes(a);player[a].hitboxes.id0=player[a].charHitboxes.throwforward.id0;
randomShout();aS[87].main(a)},main:function(a){player[a].timer++;aS[87].interrupt(a)||13!=player[a].timer||(hitQueue.push([player[a].phys.grabbing,a,0,!1,!0,!0]),turnOffHitboxes(a))},interrupt:function(a){return 27<player[a].timer?(player[a].phys.grabbing=-1,aS[0].init(a),!0):13>player[a].timer&&player[player[a].phys.grabbing].phys.grabbedBy!=a?(aS[78].init(a),!0):!1}},{name:"THROWNFORWARD",canEdgeCancel:!1,canGrabLedge:[!1,!1],init:function(a){player[a].actionState=88;player[a].timer=player[a].phys.grabbedBy<
a?-1:0;player[a].phys.grounded=!1;player[a].phys.pos=new Vec2D(player[player[a].phys.grabbedBy].phys.pos.x,player[player[a].phys.grabbedBy].phys.pos.y);aS[88].main(a)},main:function(a){player[a].timer++;aS[88].interrupt(a)},interrupt:function(a){return!1}},{name:"WALLDAMAGE",canPassThrough:!1,canGrabLedge:[!1,!1],wallJumpAble:!1,init:function(a){player[a].actionState=89;player[a].timer=0;playSfx("bounce");aS[89].main(a)},main:function(a){player[a].timer++;0==player[a].hit.hitstun%10&&drawVfx("flyingDust",
player[a].phys.pos);2==player[a].timer&&(player[a].phys.kVel.y*=.8,player[a].phys.kVel.x*=-.8,player[a].phys.kDec.x*=-1);aS[89].interrupt(a)||(player[a].hit.hitstun--,player[a].phys.cVel.y-=player[a].charAttributes.gravity,player[a].phys.cVel.y<-player[a].charAttributes.terminalV&&(player[a].phys.cVel.y=-player[a].charAttributes.terminalV))},interrupt:function(a){return 50<player[a].timer?(aS[40].init(a),!0):!1}},{name:"WALLTECH",canPassThrough:!0,canGrabLedge:[!0,!1],wallJumpAble:!0,init:function(a){player[a].actionState=
90;player[a].timer=0;player[a].phys.fastfalled=!1;player[a].hit.hitstun=0;player[a].phys.kVel.y=0;player[a].phys.kVel.x=0;player[a].phys.cVel.x=0;player[a].phys.intangibleTimer=10;1==player[a].phys.face?drawVfx("tech",player[a].phys.ECBp[3]):drawVfx("tech",player[a].phys.ECBp[1]);aS[90].main(a)},main:function(a){1>player[a].timer?(player[a].timer+=.15,1<player[a].timer&&(player[a].timer=1)):player[a].timer++;aS[90].interrupt(a)||(2==player[a].timer&&playSfx("walljump"),.89<player[a].timer&&.91>player[a].timer&&
(player[a].phys.cVel.x=.5*player[a].phys.face),1<=player[a].timer&&(fastfall(a),airDrift(a)))},interrupt:function(a){var b=checkForAerials(a),d=checkForSpecials(a);return b[0]?(aS[b[1]].init(a),!0):player[a].inputs.l[0]&&!player[a].inputs.l[1]||player[a].inputs.r[0]&&!player[a].inputs.r[1]?(aS[11].init(a),!0):(player[a].inputs.x[0]&&!player[a].inputs.x[1]||player[a].inputs.y[0]&&!player[a].inputs.y[1]||.7<player[a].inputs.lStickAxis[0].y&&.7>=player[a].inputs.lStickAxis[1].y)&&!player[a].phys.doubleJumped?
(aS[19].init(a),!0):d[0]?(aS[d[1]].init(a),!0):26<player[a].timer?(aS[13].init(a),!0):!1}},{name:"WALLJUMP",canPassThrough:!0,canGrabLedge:[!0,!1],wallJumpAble:!0,init:function(a){player[a].actionState=91;player[a].timer=0;player[a].phys.fastfalled=!1;player[a].hit.hitstun=0;player[a].phys.cVel.x=0;player[a].phys.cVel.y=0;player[a].phys.intangibleTimer=10;player[a].phys.cVel.x=1.3*player[a].phys.face;player[a].phys.cVel.y=2.4;player[a].hit.hitlag=5;player[a].hit.knockback=0;1==player[a].phys.face?
drawVfx("tech",player[a].phys.ECBp[3]):drawVfx("tech",player[a].phys.ECBp[1]);aS[91].main(a)},main:function(a){player[a].timer++;2==player[a].timer&&playSfx("walljump");aS[91].interrupt(a)||(fastfall(a),airDrift(a))},interrupt:function(a){var b=checkForAerials(a),d=checkForSpecials(a);return b[0]?(aS[b[1]].init(a),!0):player[a].inputs.l[0]&&!player[a].inputs.l[1]||player[a].inputs.r[0]&&!player[a].inputs.r[1]?(aS[11].init(a),!0):(player[a].inputs.x[0]&&!player[a].inputs.x[1]||player[a].inputs.y[0]&&
!player[a].inputs.y[1]||.7<player[a].inputs.lStickAxis[0].y&&.7>=player[a].inputs.lStickAxis[1].y)&&!player[a].phys.doubleJumped?(aS[19].init(a),!0):d[0]?(aS[d[1]].init(a),!0):40<player[a].timer?(aS[13].init(a),!0):!1}},{name:"WALLTECHJUMP",canPassThrough:!0,canGrabLedge:[!0,!1],wallJumpAble:!0,init:function(a){player[a].actionState=92;player[a].timer=0;player[a].phys.fastfalled=!1;player[a].hit.hitstun=0;player[a].phys.kVel.y=0;player[a].phys.kVel.x=0;player[a].phys.cVel.x=0;player[a].phys.cVel.y=
0;player[a].phys.intangibleTimer=10;1==player[a].phys.face?drawVfx("tech",player[a].phys.ECBp[3]):drawVfx("tech",player[a].phys.ECBp[1]);aS[92].main(a)},main:function(a){1>player[a].timer?(player[a].timer+=.15,1<player[a].timer&&(player[a].timer=1)):player[a].timer++;2==player[a].timer&&playSfx("walljump");aS[90].interrupt(a)||(.89<player[a].timer&&.91>player[a].timer&&(player[a].phys.cVel.x=1.3*player[a].phys.face,player[a].phys.cVel.y=2.4),1<=player[a].timer&&(fastfall(a),airDrift(a)))},interrupt:function(a){var b=
checkForAerials(a),d=checkForSpecials(a);return b[0]?(aS[b[1]].init(a),!0):player[a].inputs.l[0]&&!player[a].inputs.l[1]||player[a].inputs.r[0]&&!player[a].inputs.r[1]?(aS[11].init(a),!0):(player[a].inputs.x[0]&&!player[a].inputs.x[1]||player[a].inputs.y[0]&&!player[a].inputs.y[1]||.7<player[a].inputs.lStickAxis[0].y&&.7>=player[a].inputs.lStickAxis[1].y)&&!player[a].phys.doubleJumped?(aS[19].init(a),!0):d[0]?(aS[d[1]].init(a),!0):40<player[a].timer?(aS[13].init(a),!0):!1}},{name:"SLEEP",init:function(a){player[a].actionState=
93;player[a].timer=0;player[a].hit.hitstun=0;player[a].phys.kVel.y=0;player[a].phys.kVel.x=0;player[a].phys.cVel.x=0;player[a].phys.cVel.y=0;player[a].phys.pos.x=300;aS[93].main(a)},main:function(a){},interrupt:function(a){return!1}},{name:"ENTRANCE",init:function(a){player[a].actionState=94;player[a].timer=0;player[a].phys.grounded=!1;aS[94].main(a)},main:function(a){player[a].timer++;aS[94].interrupt(a)},interrupt:function(a){60<player[a].timer&&aS[13].init(a)}},{name:"CLIFFGETUPQUICK",offset:[[-71.33,
-23.71],[-71.38,-23.71],[-71.42,-23.71],[-71.45,-23.71],[-71.46,-23.71],[-71.44,-23.71],[-71.38,-23.71],[-71.26,-23.71],[-71.07,-22.69],[-70.8,-19.99],[-70.47,-16.19],[-70.11,-11.83],[-69.71,-7.48],[-69.28,-3.68],[-68.83,-1.01],[-67.88,0],[-67.38,0],[-66.87,0],[-66.35,0],[-65.81,0],[-65.27,0],[-64.73,0],[-64.19,0],[-63.65,0],[-63.12,0],[-62.59,0],[-62.08,0],[-61.6,0],[-61.17,0],[-60.8,0],[-60.5,0],[-60.28,0]],init:function(a){player[a].actionState=95;player[a].timer=0;player[a].phys.intangibleTimer=
30;aS[95].main(a)},main:function(a){player[a].timer++;aS[95].interrupt(a)||(16>player[a].timer?player[a].phys.pos=new Vec2D(aS[95].offset[player[a].timer-1][0]*player[a].phys.face,aS[95].offset[player[a].timer-1][1]):player[a].phys.pos.x=aS[95].offset[player[a].timer-1][0]*player[a].phys.face,16==player[a].timer&&(player[a].phys.grounded=!0,player[a].phys.onSurface=-1,player[a].phys.airborneTimer=0,player[a].phys.pos.y=0))},interrupt:function(a){return 32<player[a].timer?(player[a].phys.onLedge=-1,
player[a].phys.ledgeRegrabCount=!0,aS[0].init(a),!0):!1}}];twoPi=2*Math.PI;function drawArrayPath(a,b,d,e,f,n,q){for(var h=0;h<f.length;h++){var m=f[h][0]*n*b+d,g=f[h][1]*q+e;0==h?(c.fillStyle=a,c.beginPath(),c.moveTo(m,g)):c.lineTo(m,g)}c.closePath();c.fill()}
...
function findPlayers(){var a=navigator.getGamepads();keyboardOccupied||(2>gameMode?keys[109]&&4>ports&&(gameMode=1,keyboardOccupied=!0,playSfx("menuForward"),0==ports&&sounds.menu2.play("menu2Start"),addPlayer(ports,4)):keys[101]&&4>ports&&(keyboardOccupied=!0,addPlayer(ports,4)));for(var b=0;b<a.length;b++){var d=navigator.getGamepads()[b];if("undefined"!=typeof d){var e=0;"v"==d.id[0]||"1"==d.id[0]?e=1:"M"!=d.id[0]&&("G"==d.id[0]||"2"==d.id[0]?(e=2,console.log("You are using raphnet")):"X"==d.id[0]&&
(e=3,console.log("You are using xbox 360")));if(2>gameMode){if(d.buttons[map.s[e]].pressed){for(var d=!1,f=0;f<ports;f++)currentPlayers[f]==b&&(d=!0);!d&&4>ports&&(gameMode=1,playSfx("menuForward"),0==ports&&sounds.menu2.play("menu2Start"),addPlayer(b,e))}}else if(d.buttons[map.a[e]].pressed){d=!1;for(f=0;f<ports;f++)currentPlayers[f]==b&&(d=!0);d||4>ports&&addPlayer(b,e)}}}}function addPlayer(a,b){ports++;currentPlayers[ports-1]=a;playerType[ports-1]=0;mType[ports-1]=b}
...
(cd[a].ls=new Vec2D(m.axes[0],-1*m.axes[1]),cd[a].cs=new Vec2D(m.axes[5],-1*m.axes[2]),cd[a].l=m.axes[3]+.8,cd[a].r=m.axes[4]+.8,$("#resetIndicator"+a).fadeIn(100),$("#resetIndicator"+a).fadeOut(500));attemptingControllerReset[a]=!1},2E3));pause[a][0]&&!pause[a][1]&&3==gameMode&&((playing^=1)?sounds.battlefield.volume(.5):(playSfx("pause"),sounds.battlefield.volume(.2),renderForeground()));0<player[a].phys.lCancelTimer&&(player[a].phys.lCancelTimer--,0==player[a].phys.lCancelTimer&&(player[a].phys.lCancel=
!1));0==player[a].phys.lCancelTimer&&(0<player[a].inputs.lAnalog[0]&&0==player[a].inputs.lAnalog[1]||0<player[a].inputs.rAnalog[0]&&0==player[a].inputs.lAnalog[1]||player[a].inputs.z[0]&&!player[a].inputs.z[1])&&(player[a].phys.lCancelTimer=7,player[a].phys.lCancel=!0);$("#lsAxisX"+a).empty().append(d.toFixed(5));$("#lsAxisY"+a).empty().append(e.toFixed(5));$("#lAnalog"+a).empty().append(q.toFixed(5));$("#rAnalog"+a).empty().append(h.toFixed(5));if(4==mType[a])for(g=0;12>g;g++)keys[keyboardMap[g]]?
$("#"+a+"button"+g).show():$("#"+a+"button"+g).hide();else for(g=0;12>g;g++)d=g,0==mType[a]&&(5<d&&d++,7<d&&d++,8<d&&(d+=2)),m.buttons[d].pressed?$("#"+a+"button"+g).show():$("#"+a+"button"+g).hide()}
...
!0}
function endGame(){sounds.battlefield.stop();$("#matchTimer").hide();playing=!1;c.fillStyle="rgba(0, 0, 0, 1)";c.fillRect(-100,-100,canvas.width+200,canvas.height+200);drawStage();$(".stockPercent").hide();gameMode=2;sounds.menu2.play("menu2Start");pause=[[!0,!0],[!0,!0],[!0,!0],[!0,!0]];frameAdvance=[[!0,!0],[!0,!0],[!0,!0],[!0,!0]];findingPlayers=!0;for(var a=0;4>a;a++)-1<playerType[a]&&(72==player[a].actionState&&sounds.furaloop.stop(player[a].furaLoopID),player[a].inputs.a[0]=!0,player[a].inputs.a[1]=!0,
player[a].inCSS=!0,player[a].phys.face=1,player[a].actionState=0,player[a].timer=0)}
...
2500)}
$(document).ready(function(){$(".stockPercent").hide();$("#keyboardButton").click(function(){$("#keyboardControlsImg").toggle()});$("#controllerButton").click(function(){$("#controllerSupportContainer").toggle()});canvas=document.getElementById("gameCanvas");c=canvas.getContext("2d");c.fillStyle="rgba(0, 0, 0, 1)";c.fillRect(-100,-100,canvas.width+200,canvas.height+200);drawStartScreen();gameTick();renderTick();var a=Math.max(($(window).width()-1200)/2,0),b=Math.max(($(window).height()-750)/2,0);
$("#display").css("margin",b+"px 0px 0px "+a+"px");$("#effectsButton").click(function(){showVfx?$("#effectsButtonEdit").empty().append("OFF"):$("#effectsButtonEdit").empty().append("ON");showVfx^=1});$("#debugButton").click(function(){if(showDebug){$("#debugButtonEdit").empty().append("OFF");$("#debug").hide();$("#players").hide();var b=Math.max(($(window).height()-750)/2,0);$("#display").css("margin",b+"px 0px 0px "+a+"px")}else $("#debugButtonEdit").empty().append("ON"),$("#debug").show(),$("#players").show(),
b=Math.max(($(window).height()-900)/2,0),$("#display").css("margin",b+" 0px 0px px "+a+"px");showDebug^=1})});
