...
map={a:[1,0],b:[2,1],x:[0,2],y:[3,3],z:[7,4],r:[5,5],l:[4,6],s:[9,7],du:[12,8],dr:[13,11],dd:[14,9],dl:[15,10],lsX:[0,0],lsY:[1,1],csX:[5,3],csY:[2,4],lA:[3,2],rA:[4,5]};mType=[0,0,0,0];cd=[new customDeadzone,new customDeadzone,new customDeadzone,new customDeadzone];currentPlayers=[];playerAmount=0;frameByFrame=playing=!1;showVfx=findingPlayers=!0;showDebug=!1;charSelect=["marth","marth","marth","marth"];randomTags="NEO! SELF NOVA PNDA Panda LFFN Scorp AZ AXE Tempo TMPO [A]rmada WBALLZ Westballz PPMD Kreygasm M2K Mang0 USA SCAR TOPH (.Y.) HBOX HungryBox PLUP Shroomed SFAT Wizz Lucky S2J SilentWolf aMSa S2J Hax$".split(" ");
vfxQueue=[];
...
function findPlayers(){for(var a=navigator.getGamepads(),b=0;b<a.length;b++){var d=navigator.getGamepads()[b];if("undefined"!=typeof d){var e=0;if("v"==d.id[0]||"1"==d.id[0])e=1;if(d.buttons[map.a[e]].pressed){for(var f=!1,k=0;k<playerAmount;k++)currentPlayers[k]==b&&(f=!0);f||4>playerAmount&&addPlayer(b,e)}for(e=0;e<currentPlayers.length;e++)if(currentPlayers[e]==b&&(!d.buttons[map.x[mType[e]]].pressed&&!d.buttons[map.y[mType[e]]].pressed||costumeTimeout[e]||(pPal[e]++,6<pPal[e]&&(pPal[e]=0),$("#pBox"+
e).css({"background-color":palettes[pPal[e]][0],border:"5px solid "+palettes[pPal[e]][2]+"0.8)"}),costumeTimeout[e]=!0,costumeTout(e)),d.buttons[map.b[mType[e]]].pressed?(bPress[e]||(hasTag[e]^=1,hasTag[e]?($("#pTag"+e).show(),$("#pTagEdit"+e).val(randomTags[Math.round((randomTags.length-1)*Math.random())]),tagText[e]=$("#pTagEdit"+e).val()):$("#pTag"+e).hide()),bPress[e]=!0):bPress[e]=!1,d.buttons[map.z[mType[e]]].pressed?(zPress[e]||4>playerAmount&&addPlayer(-1),zPress[e]=!0):zPress[e]=!1,d.buttons[map.s[mType[e]]].pressed)){$("#playerFind").hide();
for(f=0;f<playerAmount;f++)initializePlayers(f),renderPlayer(f);findingPlayers=!1}}}}
...
1);player[a].inputs.r[0]&&(player[a].inputs.rAnalog[0]=1);player[a].inputs.dpadleft[0]=d.buttons[map.dl[mType[a]]].pressed;player[a].inputs.dpaddown[0]=d.buttons[map.dd[mType[a]]].pressed;player[a].inputs.dpadright[0]=d.buttons[map.dr[mType[a]]].pressed;player[a].inputs.dpadup[0]=d.buttons[map.du[mType[a]]].pressed}else frameAdvance[a][0]&&!frameAdvance[a][1]&&(frameByFrame=!0);player[a].inputs.dpadleft[0]&&!player[a].inputs.dpadleft[1]&&(player[a].showLedgeGrabBox^=1);player[a].inputs.dpaddown[0]&&
!player[a].inputs.dpaddown[1]&&(player[a].showECB^=1);player[a].inputs.dpadright[0]&&!player[a].inputs.dpadright[1]&&(player[a].showHitbox^=1);d.buttons[map.du[mType[a]]].pressed&&d.buttons[map.x[mType[a]]].pressed&&d.buttons[map.y[mType[a]]].pressed&&!attemptingControllerReset[a]&&(attemptingControllerReset[a]=!0,setTimeout(function(){d.buttons[map.du[mType[a]]].pressed&&d.buttons[map.x[mType[a]]].pressed&&d.buttons[map.y[mType[a]]].pressed&&(cd[a].ls=new Vec2D(d.axes[0],-1*d.axes[1]),cd[a].cs=new Vec2D(d.axes[5],
-1*d.axes[2]),cd[a].l=d.axes[3]+.8,cd[a].r=d.axes[4]+.8,$("#resetIndicator"+a).fadeIn(100),$("#resetIndicator"+a).fadeOut(500));attemptingControllerReset[a]=!1},2E3));pause[a][0]&&!pause[a][1]&&(playing^=1);0<player[a].phys.lCancelTimer&&(player[a].phys.lCancelTimer--,0==player[a].phys.lCancelTimer&&(player[a].phys.lCancel=!1));0==player[a].phys.lCancelTimer&&(0<player[a].inputs.lAnalog[0]&&0==player[a].inputs.lAnalog[1]||0<player[a].inputs.rAnalog[0]&&0==player[a].inputs.lAnalog[1]||player[a].inputs.z[0]&&
!player[a].inputs.z[1])&&(player[a].phys.lCancelTimer=7,player[a].phys.lCancel=!0);$("#lsAxisX"+a).empty().append(e.toFixed(5));$("#lsAxisY"+a).empty().append(f.toFixed(5));$("#lAnalog"+a).empty().append(h.toFixed(5));$("#rAnalog"+a).empty().append(m.toFixed(5));for(g=0;12>g;g++)e=g,0==mType[a]&&(5<e&&e++,7<e&&e++,8<e&&(e+=2)),d.buttons[e].pressed?$("#"+a+"button"+g).show():$("#"+a+"button"+g).hide()}
