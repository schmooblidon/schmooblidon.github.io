...
function findPlayers(){for(var a=navigator.getGamepads(),b=0;b<a.length;b++){var d=navigator.getGamepads()[b];if("undefined"!=typeof d){var e=0;"v"==d.id[0]&&(e=1);if(d.buttons[map.a[e]].pressed){for(var f=!1,k=0;k<playerAmount;k++)currentPlayers[k]==b&&(f=!0);f||4>playerAmount&&addPlayer(b,e)}for(e=0;e<currentPlayers.length;e++)if(currentPlayers[e]==b&&(!d.buttons[map.x[mType[e]]].pressed&&!d.buttons[map.y[mType[e]]].pressed||costumeTimeout[e]||(pPal[e]++,6<pPal[e]&&(pPal[e]=0),$("#pBox"+e).css({"background-color":palettes[pPal[e]][0],
border:"5px solid "+palettes[pPal[e]][2]+"0.8)"}),costumeTimeout[e]=!0,costumeTout(e)),d.buttons[map.b[mType[e]]].pressed?(bPress[e]||(hasTag[e]^=1,hasTag[e]?($("#pTag"+e).show(),$("#pTagEdit"+e).val(randomTags[Math.round((randomTags.length-1)*Math.random())]),tagText[e]=$("#pTagEdit"+e).val()):$("#pTag"+e).hide()),bPress[e]=!0):bPress[e]=!1,d.buttons[map.z[mType[e]]].pressed?(zPress[e]||4>playerAmount&&addPlayer(-1),zPress[e]=!0):zPress[e]=!1,d.buttons[map.s[mType[e]]].pressed)){$("#playerFind").hide();
...
function interpretInputs(a,b){var d=navigator.getGamepads()[currentPlayers[a]];console.log(d.axes[0]);var e=d.axes[0]-cd[a].ls.x,f=-1*d.axes[1]-cd[a].ls.y,e=e/.75,f=f/.75,e=1<e?1:-1>e?-1:Math.round(80*e)/80,f=1<f?1:-1>f?-1:Math.round(80*f)/80;.3>Math.abs(e)&&(e=0);.3>Math.abs(f)&&(f=0);var k=-1*d.axes[2]-cd[a].cs.y,l=d.axes[5]-cd[a].cs.x,l=l/.75,k=k/.75,l=1<l?1:-1>l?-1:Math.round(80*l)/80,k=1<k?1:-1>k?-1:Math.round(80*k)/80;.3>Math.abs(l)&&(l=0);.3>Math.abs(k)&&(k=0);var h=d.axes[3]-cd[a].l,m=d.axes[4]-
cd[a].r,h=h+.9,m=m+.9;1<h&&(h=1);1<m&&(m=1);.3>h&&(h=0);.3>m&&(m=0);pause[a][1]=pause[a][0];frameAdvance[a][1]=frameAdvance[a][0];pause[a][0]=d.buttons[map.s[mType[a]]].pressed?!0:!1;frameAdvance[a][0]=d.buttons[map.z[mType[a]]].pressed?!0:!1;if(b){for(var g=0;7>g;g++)player[a].inputs.lStickAxis[7-g].x=player[a].inputs.lStickAxis[6-g].x,player[a].inputs.lStickAxis[7-g].y=player[a].inputs.lStickAxis[6-g].y,player[a].inputs.cStickAxis[7-g].x=player[a].inputs.cStickAxis[6-g].x,player[a].inputs.cStickAxis[7-
g].y=player[a].inputs.cStickAxis[6-g].y,player[a].inputs.lAnalog[7-g]=player[a].inputs.lAnalog[6-g],player[a].inputs.rAnalog[7-g]=player[a].inputs.rAnalog[6-g],player[a].inputs.z[7-g]=player[a].inputs.z[6-g],player[a].inputs.a[7-g]=player[a].inputs.a[6-g],player[a].inputs.b[7-g]=player[a].inputs.b[6-g],player[a].inputs.x[7-g]=player[a].inputs.x[6-g],player[a].inputs.y[7-g]=player[a].inputs.y[6-g],player[a].inputs.r[7-g]=player[a].inputs.r[6-g],player[a].inputs.l[7-g]=player[a].inputs.l[6-g],player[a].inputs.dpadleft[7-
g]=player[a].inputs.dpadleft[6-g],player[a].inputs.dpaddown[7-g]=player[a].inputs.dpaddown[6-g],player[a].inputs.dpadright[7-g]=player[a].inputs.dpadright[6-g],player[a].inputs.dpadup[7-g]=player[a].inputs.dpadup[6-g];player[a].inputs.lStickAxis[0].x=e;player[a].inputs.lStickAxis[0].y=f;player[a].inputs.cStickAxis[0].x=l;player[a].inputs.cStickAxis[0].y=k;player[a].inputs.lAnalog[0]=h;player[a].inputs.rAnalog[0]=m;player[a].inputs.x[0]=d.buttons[map.x[mType[a]]].pressed;player[a].inputs.a[0]=d.buttons[map.a[mType[a]]].pressed;
