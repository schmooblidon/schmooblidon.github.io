...
tagText=["","","",""];pPal=[];costumeTimeout=[];colours=["rgba(4, 255, 82, 0.62)","rgba(117, 20, 255, 0.63)","rgba(255, 20, 20, 0.63)","rgba(255, 232, 20, 0.63)"];pause=[[!1,!1],[!1,!1]];frameAdvance=[[!1,!1],[!1,!1]];startingPoint=[[-50,0],[50,0],[-25,0],[25,0]];startingFace=[1,-1,-1,1];ground=[[-68.4,0],[68.4,0]];platforms=[[[-57.6,27.2],[-20,27.2]],[[20,27.2],[57.6,27.2]],[[-18.8,54.4],[18.8,54.4]]];wallsL=[[[-68.4,0],[-68.4,-108.8]]];wallsR=[[[68.4,0],[68.4,-108.8]]];
...
function findPlayers(){for(var a=navigator.getGamepads(),b=0;b<a.length;b++){var d=navigator.getGamepads()[b];if("undefined"!=typeof d){if(d.buttons[1].pressed){for(var e=!1,f=0;f<playerAmount;f++)currentPlayers[f]==b&&(e=!0);e||4>playerAmount&&addPlayer(b)}for(e=0;e<currentPlayers.length;e++)if(currentPlayers[e]==b&&(!d.buttons[0].pressed&&!d.buttons[3].pressed||costumeTimeout[e]||(pPal[e]++,6<pPal[e]&&(pPal[e]=0),$("#pBox"+e).css({"background-color":palettes[pPal[e]][0],border:"5px solid "+palettes[pPal[e]][2]+
"0.8)"}),costumeTimeout[e]=!0,costumeTout(e)),d.buttons[2].pressed?(bPress[e]||(hasTag[e]^=1,hasTag[e]?($("#pTag"+e).show(),$("#pTagEdit"+e).val(randomTags[Math.round((randomTags.length-1)*Math.random())]),tagText[e]=$("#pTagEdit"+e).val()):$("#pTag"+e).hide()),bPress[e]=!0):bPress[e]=!1,d.buttons[7].pressed?(zPress[e]||4>playerAmount&&addPlayer(-1),zPress[e]=!0):zPress[e]=!1,d.buttons[9].pressed)){$("#playerFind").hide();for(f=0;f<playerAmount;f++)initializePlayers(f),renderPlayer(f);findingPlayers=
!1}}}}
function addPlayer(a){currentPlayers.push(a);playerAmount++;costumeTimeout.push(!1);pPal.push(playerAmount-1);$("#currentPlayers").append('<div class="pBoxBox"><div class="playerBox" id="pBox'+(playerAmount-1)+'" style="background-color:'+palettes[pPal[playerAmount-1]][0]+";border:5px solid "+palettes[pPal[playerAmount-1]][2]+'0.8)"><p>P'+playerAmount+'<br><span class="cont">(Cont. '+a+')</span></p></div><div id="pTag'+(playerAmount-1)+'" class="pTag"><textarea id="pTagEdit'+(playerAmount-1)+'" class="pTagEdit" maxlength="10"></textarea></div></div>')}
...
1<h&&(h=1);1<m&&(m=1);.3>h&&(h=0);.3>m&&(m=0);pause[a][1]=pause[a][0];frameAdvance[a][1]=frameAdvance[a][0];pause[a][0]=d.buttons[9].pressed?!0:!1;frameAdvance[a][0]=d.buttons[7].pressed?!0:!1;if(b){for(var g=0;7>g;g++)player[a].inputs.lStickAxis[7-g].x=player[a].inputs.lStickAxis[6-g].x,player[a].inputs.lStickAxis[7-g].y=player[a].inputs.lStickAxis[6-g].y,player[a].inputs.cStickAxis[7-g].x=player[a].inputs.cStickAxis[6-g].x,player[a].inputs.cStickAxis[7-g].y=player[a].inputs.cStickAxis[6-g].y,player[a].inputs.lAnalog[7-
g]=player[a].inputs.lAnalog[6-g],player[a].inputs.rAnalog[7-g]=player[a].inputs.rAnalog[6-g],player[a].inputs.z[7-g]=player[a].inputs.z[6-g],player[a].inputs.a[7-g]=player[a].inputs.a[6-g],player[a].inputs.b[7-g]=player[a].inputs.b[6-g],player[a].inputs.x[7-g]=player[a].inputs.x[6-g],player[a].inputs.y[7-g]=player[a].inputs.y[6-g],player[a].inputs.r[7-g]=player[a].inputs.r[6-g],player[a].inputs.l[7-g]=player[a].inputs.l[6-g],player[a].inputs.dpadleft[7-g]=player[a].inputs.dpadleft[6-g],player[a].inputs.dpaddown[7-
g]=player[a].inputs.dpaddown[6-g],player[a].inputs.dpadright[7-g]=player[a].inputs.dpadright[6-g],player[a].inputs.dpadup[7-g]=player[a].inputs.dpadup[6-g];player[a].inputs.lStickAxis[0].x=e;player[a].inputs.lStickAxis[0].y=f;player[a].inputs.cStickAxis[0].x=l;player[a].inputs.cStickAxis[0].y=k;player[a].inputs.lAnalog[0]=h;player[a].inputs.rAnalog[0]=m;player[a].inputs.x[0]=d.buttons[0].pressed;player[a].inputs.a[0]=d.buttons[1].pressed;player[a].inputs.b[0]=d.buttons[2].pressed;frameByFrame||(player[a].inputs.z[0]=
d.buttons[7].pressed,player[a].inputs.z[0]&&(player[a].inputs.lAnalog[0]=.35,player[a].inputs.a[0]=!0));player[a].inputs.y[0]=d.buttons[3].pressed;player[a].inputs.r[0]=d.buttons[5].pressed;player[a].inputs.l[0]=d.buttons[4].pressed;player[a].inputs.l[0]&&(player[a].inputs.lAnalog[0]=1);player[a].inputs.r[0]&&(player[a].inputs.rAnalog[0]=1);player[a].inputs.dpadleft[0]=d.buttons[15].pressed;player[a].inputs.dpaddown[0]=d.buttons[14].pressed;player[a].inputs.dpadright[0]=d.buttons[13].pressed;player[a].inputs.dpadup[0]=
d.buttons[12].pressed}else frameAdvance[a][0]&&!frameAdvance[a][1]&&(frameByFrame=!0);player[a].inputs.dpadleft[0]&&!player[a].inputs.dpadleft[1]&&(player[a].showLedgeGrabBox^=1);player[a].inputs.dpaddown[0]&&!player[a].inputs.dpaddown[1]&&(player[a].showECB^=1);player[a].inputs.dpadright[0]&&!player[a].inputs.dpadright[1]&&(player[a].showHitbox^=1);d.buttons[12].pressed&&d.buttons[0].pressed&&d.buttons[3].pressed&&!attemptingControllerReset[a]&&(attemptingControllerReset[a]=!0,setTimeout(function(){d.buttons[12].pressed&&
d.buttons[0].pressed&&d.buttons[3].pressed&&(cd[a].ls=new Vec2D(d.axes[0],-1*d.axes[1]),cd[a].cs=new Vec2D(d.axes[5],-1*d.axes[2]),cd[a].l=d.axes[3]+.8,cd[a].r=d.axes[4]+.8,$("#resetIndicator"+a).fadeIn(100),$("#resetIndicator"+a).fadeOut(500));attemptingControllerReset[a]=!1},2E3));pause[a][0]&&!pause[a][1]&&(playing^=1);0<player[a].phys.lCancelTimer&&(player[a].phys.lCancelTimer--,0==player[a].phys.lCancelTimer&&(player[a].phys.lCancel=!1));0==player[a].phys.lCancelTimer&&(0<player[a].inputs.lAnalog[0]&&
0==player[a].inputs.lAnalog[1]||0<player[a].inputs.rAnalog[0]&&0==player[a].inputs.lAnalog[1]||player[a].inputs.z[0]&&!player[a].inputs.z[1])&&(player[a].phys.lCancelTimer=7,player[a].phys.lCancel=!0);$("#lsAxisX"+a).empty().append(e.toFixed(5));$("#lsAxisY"+a).empty().append(f.toFixed(5));$("#lAnalog"+a).empty().append(h.toFixed(5));$("#rAnalog"+a).empty().append(m.toFixed(5));for(g=0;16>g;g++)d.buttons[g].pressed?$("#"+a+"button"+g).show():$("#"+a+"button"+g).hide()}
...
$(document).ready(function(){$(".stockPercent").hide();canvas=document.getElementById("gameCanvas");c=canvas.getContext("2d");c.fillStyle="rgba(0, 0, 0, 1)";c.fillRect(-100,-100,canvas.width+200,canvas.height+200);drawStage();gameTick();renderTick();var a=Math.max(($(window).width()-1200)/2,0),b=Math.max(($(window).height()-750)/2,0);$("#display").css("margin",b+"px "+a+"px");$("#effectsButton").click(function(){showVfx?$("#effectsButtonEdit").empty().append("OFF"):$("#effectsButtonEdit").empty().append("ON");
showVfx^=1});$("#debugButton").click(function(){if(showDebug){$("#debugButtonEdit").empty().append("OFF");$("#debug").hide();$("#players").hide();var b=Math.max(($(window).height()-750)/2,0)}else $("#debugButtonEdit").empty().append("ON"),$("#debug").show(),$("#players").show(),b=Math.max(($(window).height()-900)/2,0);$("#display").css("margin",b+"px "+a+"px");showDebug^=1})});
